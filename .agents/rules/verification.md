...
Exercise changed code end to end through real downstream callers in a representative scenario and meaningful failure case where feasible, including fixes made during a code review. Running/inspecting the game or a small one-off integration script is the intended verification, not prohibited test-suite authoring; static checks alone are insufficient. Use disposable worlds, never replace someone's save. Native checks use `AI_BUDGET_USD=0` without unrelated credentials. UI work needs actual interaction/visual inspection when available, not only a build. Explicit findings-only reviews and design-only work do not require changing or running the game merely because a skill mentions verification. Missing access is a gap, not a pass.
