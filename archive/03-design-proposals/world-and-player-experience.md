...
Known actions feel immediate. Novel actions acknowledge quickly, show an attempt or planning state, and return a clear result or limitation. The future narration slice also permits explicitly effect-free expressive actions, such as a frown or an unsupported slap: their accepted expressive occurrence can be narrated and remembered, but no injury, health loss or other mechanical result may be invented. Use a subtle expression/no-effects affordance where needed. The event feed and visual effects must agree with authoritative outcomes; generated prose never proves a physical state change.
...
## The Narrator and personal story

Accepted future direction, September 20, 2026. **The Narrator** describes non-speech actions/reactions and relevant world events during conversation, and significant standalone moments such as meeting an important person or entering a village for the first time. Conversation prose sits between speech; standalone prose appears across the top of the screen as if written in a book and remains in the player's private Journal. A player setting selects narrator voice. Embellishment affects storytelling style, not facts or effects.

If supported rules really commit a slap, renewed eye-wound bleeding and one health lost, the conversation can read “Ada slaps you. Your eye wound from yesterday starts bleeding.” Beneath it, smaller distinct text shows “−1 Health.” Every factual clause and impact needs permitted source evidence; private thoughts are excluded. Initially an unsupported slap is expressive only, with no bleeding or health impact. On-the-spot invention of missing conditional action/effect rules is explicitly deferred.

External occurrences share one master world-event record and actor/event awareness joins. Retain witnessed events and notable unseen events selected by a configured importance score; discard ordinary unseen experiential records without losing simulation state. Notability alone does not reveal facts. An explicitly permitted dramatic cutaway may say “Far away, a demon awakens,” but it does not teach the player's character that fact. Single-actor narration may include private fictional thoughts only under an explicit access grant.

Narrations are private descriptions stored separately from world events, with explicit many-to-many source links. A meteor affecting several conversations has one unbound world event and separate appropriate narrations for aware players in each conversation. Durable conversation history combines its permitted speech/action events with its private narration, in stable chronological order. Listening, joining and history access are distinct; see [social continuity](agents-and-social-simulation.md#social-continuity), the [technical design](../../docs/narration-and-conversations.md), and its [NC task tracker](../../docs/narration-and-conversations.md#13-implementation-tasks). None of this future feature is implemented by the documentation update.
...
Generic transformations create opportunity: gather, cut, bind, assemble, consume, repair, ignite, extinguish. Seed the fundamental survival variants rather than waiting for first-use generation to make survival possible. A tree yields a bounded amount of wood, with waste and regrowth policy. A shelter plan consumes material and work to assemble editable walls, roof sections, supports, and other needed parts. Parts retain their own material, condition, and connections; coverage and usable spaces derive from their arrangement. Start with a grid, approved part shapes, simple support rules, and staged construction visuals. Expansion, demolition, and material replacement modify the same dwelling rather than converting an indivisible prefab tier. Household use and comfortable occupancy derive from actual spaces and residents. [Evolving materials and construction](evolving-materials-and-construction.md) specifies this direction; [heat/fire](heat-and-fire.md) connects local material state to ignition, spread, and damage. Arbitrary geometry, detailed structural collapse, and terrain deformation can follow separately.
