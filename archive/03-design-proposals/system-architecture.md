...
Each NPC has a logical persistent mind in shared storage, with [separate records and bounded retrieval](memory-storage-and-retrieval.md). A full Macrofold workspace is an optional execution/artifact layer for tasks that benefit from its harness, not a prerequisite for rich memory. See the [repository comparison and cost scenarios](../02-research/macrofold-workspaces.md). This is a recommendation pending D28; no adapter or storage schema is implemented.
