...
The creator can talk to a world agent that can look up the world's environment, all characters and their contextual records, current and historical state, inventions and the entire world log. It can answer what happened, explain mechanics, author inventions and workshop existing ones. Requests such as making something burn slower should not require manually rewriting a declaration.
...
| 4 | Ask the creator agent about authorized world-wide state/history, inspect evidence and retain multi-turn drafts | INV-4, alongside INV-3 |
