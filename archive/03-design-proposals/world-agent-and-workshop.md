...
Creator access excludes human-private messages and private character notes, including their derived summaries, search snippets, counts and exports. Broad world inspection does not override participant/owner permissions. Important events and summaries may outlive routine detail; the agent must distinguish expired exact evidence from no event and cannot regenerate a supposedly historical quote. [Query privacy](../07-technical-architecture/data-queries-and-mcp.md#human-private-content-boundary) and [retention](../07-technical-architecture/data-delivery-and-scale.md#4-retention-archives-and-deletion) own these September 25 decisions.

An invention opened from a log or account library offers Inspect, Workshop and, where permitted, Export / use in another world. The workshop retains the conversation, draft, base version, evidence and validation results across turns. The author can refine their own contribution; activation in another owner's world still requires that world's permissions. Accepted target: a deliberate creator edit or scoped special invention grant can authorize reviewed shared-law changes independently of the ordinary player lock; see [governance](invention-governance-and-ownership.md#special-player-invention-permission). The independent agent lock need not change. Calling the world agent or labeling a new behavior a revision cannot bypass the applicable lock: a player-directed AI workflow retains player origin, while autonomous NPC invention follows the agent lock.
...
Conjuring is a separate server-authorized creator operation that can instantiate an admitted object without ordinary crafting resources. It must not silently grant character knowledge or change unrelated objects. Recheck god access, world/policy revisions and definition readiness at commit; new behavior uses current creator/invention authority and shared admission; invention permission alone does not grant conjuring. Persist request, candidate, confirmation and one instance-creation receipt so duplicate submissions or restart cannot create extra objects or repeat paid generation.
...
7. Activate atomically at a simulation boundary with explicit migration or compatible pinned-process completion. Preserve consumed fuel, elapsed burn time and relevant state. Slower burning cannot refill fuel or undo a fire. Invalid execution or a failed commit leaves the previous version active; identified gameplay danger may instead be explicitly accepted under the [law-change risk policy](../07-technical-architecture/declarations-and-evolution.md#accepted-live-change-and-risk-policy).
...
## Accepted authoring and maintenance follow-through

The [special invention permission](invention-governance-and-ownership.md#special-player-invention-permission) extends shared-law authoring to specifically authorized players without making them world creators. The workshop must use the [law-change impact and risk contract](../07-technical-architecture/declarations-and-evolution.md#accepted-live-change-and-risk-policy): investigate consequences, recommend consistent incompatibility treatments, explain known/possible/unknown risks, and permit explicit risk acceptance. Invention permission grants neither private-human inspection nor maintenance controls. [Creator maintenance](time-and-simulation-speed.md#creator-maintenance-pause) supplies scheduled notices and world pause/resume. These are accepted targets, not claims of current UI delivery.
