...
The [unified runtime](../../docs/world-agent-runtime.md), [shared tools](../../docs/invention-workshop-tools.md), [MCP connector](../../docs/world-agent-mcp.md), [graph](../../docs/invention-graph.md) and [composition](../../docs/invention-composition.md) now specify the end-to-end technical paths. The [target journeys](../../docs/invention-scenarios.md) connect them to player and NPC behavior. The accepted session allowance includes images within $5; this target does not imply the existing Discuss transport already has MCP tools.
...
The player or creator uses one unified World Agent conversation for discussion, investigation, design and permitted actions, requesting approval only where the action policy requires it. The World Agent is an out-of-world assistant, not an NPC or a proxy for the controlled character's limited knowledge. It can look up the world's environment, all authorized character/contextual records, current and historical state, inventions and the retained authorized world log, with explicit gaps for unwitnessed or expired experiential events. Its visible results must respect the recipient's world-level grants; broad inspection never grants unrelated-world, secret, or payer access. It can answer what happened, explain mechanics, author inventions and workshop existing ones. Requests such as making something burn slower should not require manually rewriting a declaration.
...
The [application tool contract](../../docs/invention-workshop-tools.md) owns scoped discovery, inspection, draft validation and explicit Apply, including how to extend those operations to additional definition kinds. [Architecture](../../docs/architecture.md#invention-workshop-tools) records the finite recipe implementation; the broader privileged workshop below remains target behavior.
