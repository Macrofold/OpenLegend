...
Interpretation and execution may have different deadlines. Prefer reasonable context-based assumptions and automated routine execution, with a visible plan/action where useful. Ask sparingly when unresolved intent materially changes the goal or consequences; minor unspecified details should not block play. Do not let a model silently resolve “burn it” against the wrong person or building. A novel method can be evaluated while ordinary activity continues; provider delay is not automatically simulated work or permission to immobilize the actor. Reservations, partial work, cancellation, and stale-state rechecks follow declared stages. [World creation and discovery](world-creation-and-discovery.md) records the accepted direction.
