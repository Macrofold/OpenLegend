...
The accepted [sleep policy](../../docs/worlds/base/sleep.md) uses energy thresholds and daily sleep windows; the [memory design](../../docs/memory-architecture.md) retains configurable continuous-sleep dream eligibility, a six-game-hour raw recall window and hourly cleanup. Save sleep episodes and cleanup progress in deterministic state. There is no daily rest quota or sleep-debt accounting. Native sleep and conversation never wait for optional paid dreams or background reflection; missed maintenance coalesces under real-time caps.
...
Connected background progression is explicitly opted into by unchecking **Pause game when hidden**. This does not enable fully offline progression, running after server shutdown, or shared-world pause arbitration. Shared-world character absence follows the accepted [logout/protection policy](../../docs/worlds/base/lifecycle-and-protection.md#human-logout-and-return); broader time and aging tuning remains open.

Shared worlds use a world-level **Continue while unattended** setting. When enabled, the authoritative simulation continues after every player disconnects or leaves their tab unfocused, subject to ordinary load, storage, and operator controls. When disabled, one present and focused player is enough to keep the world running; the world pauses only when every admitted player is disconnected or unfocused. One player's pause control or loss of focus never pauses a shared world while another player remains present and focused. Server downtime does not become elapsed simulation time in either mode, and reconnect resumes from committed state without catch-up.

The personal-world **Pause game when hidden** preference remains a player-profile setting for the current single-player host. Shared-world unattended progression is world policy, not a per-player preference: clients report focus and connection liveness, while the authority computes the aggregate pause state. Manual world pause, storage failure, and operator controls still override presence.
...
## Creator maintenance pause

Accepted target, September 25, 2026. A world creator can pause the world for maintenance, inspect it, apply authorized changes, then explicitly resume. Pause settles at a committed boundary, stops simulation advancement and ordinary gameplay mutations, and fences stale actions/jobs; networking, notices, authorized inspection/editing and external usage reconciliation may continue. Reuse existing pause, writer and save/activation boundaries rather than a separate maintenance runtime. Maintenance overrides unattended progression and is durable across restart; no offline catch-up occurs on resume.

Creators can schedule and communicate maintenance windows, for example “World will be down for maintenance between 10pm and 12am ET.” Store unambiguous real-world start/end instants and an IANA display timezone; include dates across midnight/daylight-saving boundaries and show player-local equivalents. Publish current/scheduled/changed/cancelled notices to players. The announced end is an estimate, not permission to resume a failed or unfinished migration: only a ready world resumes, with an updated notice if delayed. Scheduled start enters the same safe pause path; a schedule does not approve law changes or paid work. Maintenance intent/schedules belong to operational control outside gameplay rewind. Track delivery in MP03.
