...
The accepted [sleep policy](../../docs/sleep.md) uses energy thresholds and daily sleep windows; the [memory design](../../docs/memory-architecture.md) retains configurable continuous-sleep dream eligibility, a six-game-hour raw recall window and hourly cleanup. Save sleep episodes and cleanup progress in deterministic state. There is no daily rest quota or sleep-debt accounting. Native sleep and conversation never wait for optional paid dreams or background reflection; missed maintenance coalesces under real-time caps.
