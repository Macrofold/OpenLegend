...
This document owns product behavior for invention locks, creator attribution/control, portability, creator libraries, world invention packs, free-use worlds and contribution permissions. Account libraries, multiplayer permissions and marketplace packs remain target behavior; see [Architecture](../../docs/architecture.md) for executable behavior.
...
The authoritative policy record must contain a world ID, revision, independent agent and player lock values, effective time, changing principal, and audit reason. Exact schema names are implementation choices; changing either value must preserve the other. Safe client projections expose both values and the current revision; owner controls label them separately, while player menus explain the player setting. A shared policy revision may version both values without coupling their behavior.
...
Accepted target: creator administration and the explicit [full invention freedom grant](#special-player-invention-permission) may authorize reviewed world-definition changes independently of ordinary invention locks. These are server-checked capabilities, not a god-mode UI bypass. The current local runtime must not be described as implementing this new grant until its admission and persistence paths are delivered.
...
Cloning a world pack creates a new world's authorized definition library. It does not copy the live population, belongings, conversations, character memories, account permissions or entire simulation save. A separate world-template/state-copy operation may later offer selected authorized starting data. Portable mechanics still need supported native primitives, compatible world rules and explicit activation in the destination. Player-directed imports require the destination's player invention lock off or explicit creator/scoped special invention authority; autonomous imports follow the agent lock and their initiating authority. An ordinary import never bypasses its applicable lock.
...
## Special player invention permission

Accepted target, September 25, 2026; implementation remains tracked in INV-1 and MP02.

Ordinary players and enabled NPCs can invent automatically within implemented, permitted mechanics after validation. New or changed shared world laws require the world creator's approval, **or an explicit full invention freedom permission on that player's profile**. That permission includes authoring and activating shared rules; it is not world-creator status. Within its explicit world scope it overrides the ordinary player invention lock and ordinary invention-policy restriction on changing laws. It never expands the autonomous NPC lock through ambiguous delegation.

Store special permissions as typed, extensible server-owned grants, with subject account, capability kind, explicit scope, grantor, grant/revocation state and revision. Profile settings display these grants; a client-editable profile flag is not authority. Use world-scoped grants initially, issued/revoked by that world's creator or authorized staff; a grant in one world grants nothing in another. The target data catalogue owns the representation. Future permission kinds must declare their own powers rather than inherit invention freedom.
