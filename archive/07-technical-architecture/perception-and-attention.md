...
**Accepted background policy:** when a connected tab is hidden and world time continues, the character retains ordinary body-based simulated senses, including sight and hearing. This is an explicit background exception to foreground screen parity, not an unattended roaming camera; hidden camera intentions cannot expand exposure. NPC senses remain independent. Resume rebuilds the current foreground approved viewport/LOS projection without fabricating missed scenes. Only one embodied view may contribute foreground camera exposure; creator inspection never teaches the character. Logout is separate and follows [inactive-character protection](../../docs/worlds/base/lifecycle-and-protection.md#human-logout-and-return). This policy remains a target until the server/client paths are qualified.
...
Accepted direction: retain the current visual blur language for vision boundaries/detail levels, and offer an analogous hearing visualization. Sight and hearing indicators can each be toggled on/off. These are presentations of authorized exposure, never permission to reveal hidden content; the hearing overlay remains a target where not implemented.
