...
This document owns the proposed perception/attention contract. Read it with [context assembly](context-and-inference.md), the [production data model](production-data-model.md), [memory](../03-design-proposals/memory-storage-and-retrieval.md), [state ownership](../03-design-proposals/state-systems-and-future-influences.md), [heat/fire](../03-design-proposals/heat-and-fire.md) and [time](../03-design-proposals/time-and-simulation-speed.md). These are game-owned policies behind replaceable infrastructure ports.
...
| Reconsideration        | Ada's stimulus digest combines goal, 20-point fullness/energy bands, activity, latest perceived speech and known recipes. A changed digest can schedule work no more often than the configured real-time interval, default **45 seconds**, while running, configured and otherwise eligible. Jev selects an existing action, reflection or unknown; reflection invokes an LLM. [Director](../../apps/server/src/ai-director.ts), [configuration](../../apps/server/src/config.ts) | A person entering view alone does not trigger this digest; no semantic attention subscription, exposure-delta batch or recurring low-need reminder independent of a changed digest |
...
flowchart TD
    ...
    B --> J[Bounded Jev judgment when useful]
    J --> C
    J --> L[LLM reflection or bounded agent when needed]
...
Use native code for geometry, attenuation, detail selection, novelty bookkeeping, known hazards, meter crossings and executable action checks. Use Jev aggressively for bounded semantic relevance and reaction choices that these rules cannot resolve. Use an LLM for worthwhile new expression, a difficult plan or an invention; use a tool-using agent only when bounded additional investigation is needed. Generated descriptions, tags and semantic matches never authorize effects.
...
The canonical inventory of objects can start as indexed database rows; a vector database is not required for each entity. Keep a semantic retrieval port capable of supporting arbitrary intents. Combine exact IDs, lexical search and admitted tags/affordances with embeddings or a bounded semantic reranker when wording cannot be matched adequately. Embedding only archetypes is insufficient for an instance with a distinctive relevant state; supplement it with state tags and selective instance text. A shared embedding must not encode secret or near-only facts for a far-observer query.
...
2. Retrieve a small set of **permitted concepts, supported affordances and already known candidates**. “Find something to lash a frame together” can find known flexible/binding materials without requiring a hardcoded goal named `find_rope`. If interpretation is unresolved, use a bounded Jev judgment and, only if necessary, a richer model call under the existing budget.
...
| ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A person/agent enters view                                        | Always record a logical encounter decision opportunity, even if identity is only “unrecognized person.” Native rules or a batched Jev judgment can decide to continue the present activity; a greeting or LLM thought is not compulsory |
...
| Exposure receipt           | Actor, modality, observation sequence/time, permitted perceived handle/facts, detail or audibility, recognition uncertainty, source/evidence binding | Native derivation plus server audience projection; use `mind.observations` for retained evidence |
...
| Decision context           | Permitted current evidence plus labeled historical memory, relevant goals/needs, candidate actions, omissions and versions                           | Existing application context contract                                                            |
...
5. **Arbitrary semantic interests and Jev routing.** Add the replaceable shared-definition/instance retrieval projection, selective embedding/reranking, intent-to-interest compilation, scoped batches and measured Jev escalation to LLMs. Exact tags alone are an internal milestone, not acceptance of arbitrary-intent semantic retrieval. Include index lag and budget exhaustion behavior.
