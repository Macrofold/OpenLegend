...
Begin with ordinary relational tables and schema-governed JSON. Index owner/world/entity/status/time and use full-text search for prose; required semantic-retrieval vectors remain derived indexes and never establish knowledge, rights or truth. The current memory implementation uses OpenAI `text-embedding-3-small` (512 dimensions) and pgvector exact cosine top-N search inside PostgreSQL; only permitted IDs/scores return to the application. Approximate indexing remains a measured scaling option. One transactional store owns the initial world writer's authoritative state, commit batches, receipts and necessary durable asynchronous work. General distributed outboxes, renewable leases and generic checkpoint replay are conditional expansions under the [production scope](production-data-model.md#implementation-scope-baseline-versus-conditional-expansion), not initial service requirements. In-memory state is its working projection, not a competing persistence authority.
