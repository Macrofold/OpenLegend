...
| `memory` | Episodes, beliefs, relationships, commitments, skills and recall/forgetting policy | Scoped retrieval and versioned memory proposals |
...
| Observations/minds/memories | Per-actor permitted experience, beliefs, commitments, goals and retention; [detailed design](../03-design-proposals/memory-storage-and-retrieval.md) |
...
Begin with ordinary relational tables and schema-governed JSON. Index owner/world/entity/status/time and use full-text search for prose; optional vectors never establish knowledge, rights or truth. One transactional store owns the initial sector's authoritative state, commit batches, receipts and outbox. In-memory state is its working projection, not a competing persistence authority.
...
The [context specification](context-and-inference.md) defines construction stages, unknown/omitted data, authorization, dependency tracking, routing and caching. The execution adapter receives that prepared context and a versioned task definition, not permission to browse arbitrary application storage. Approved tools can fetch additional authorized evidence. The domain module decides what that evidence means.
