...
A million-player ambition is a direction, not an adequately specified load test. [D61](../05-project/open-decisions.md#d61--shared-world-scale-and-crowd-behavior) owns the remaining workload and crowd policy choices. Record at least:
...
Nearby regions exchange only the bounded boundary state needed for supported interactions, with source revisions and simulation-time validity. Seeing/hearing across an edge does not permit using a stale replica to spend another region's inventory. World policy versions have explicit activation boundaries, not a global lock for every movement. Cross-region trades and physical effects must declare coordination, retry and unavailable behavior before release. If a concentrated crowd exceeds sustainable interaction/delivery work, the allowed entry, detail or slowdown response is a product decision under D61; the database cannot make all-to-all interaction free.
...
Accepted direction: preserve important events and summaries, and allow routine detail to age out. Character forgetting is a separate fictional policy. Exact age/byte limits, protected event classes and summary coverage remain [D59](../05-project/open-decisions.md#d59--historical-retention-and-command-retry-horizon); no deletion job is authorized by an unspecified horizon. The following numeric windows are unaccepted planning candidates. Their adoption requires measured cost and chosen product terms, and changes historical-query coverage.
...
| Routine experiential detail | Finite hot/optional archive window, then expire only after required evidence/checkpoint/receipt dependencies are satisfied. Exact window remains D59; no promise that every recorded event stays searchable forever. |
