...
## 2. First hosted deployment

Start with one PostgreSQL 18 cluster, a primary and an appropriate managed high-availability configuration, one application deployment with separate simulation/query/background worker pools, and an object store. PostgreSQL is the canonical transactional store; memory caches and the renderer are disposable projections. Services may initially share a codebase and host, with separate pools and credentials. Do not provision every eventual component just because the model has a namespace for it.

Run worlds under explicit authority leases/epochs. Begin with one sector per world, independently assigned to a worker. Multiple worlds can run on one worker until measurements justify spreading them. Database connections belong to bounded service pools, not one connection per player, actor or MCP call. Backpressure must cap requests and queue age before memory or connection exhaustion.
...
High availability has explicit acknowledged-write semantics. Synchronous replication can reduce failover loss at a latency/availability cost; asynchronous replication can lose acknowledged recent commits during promotion. Select and test the launch policy. PostgreSQL describes these tradeoffs in [standby replication](https://www.postgresql.org/docs/18/warm-standby.html). Fencing must cover both simulation workers and database failover so an old authority cannot return and write stale state.
...
| Ordinary meaningful world journal                                   | 30 days of real-time hot data, then compressed queryable archive while the world exists and its configured storage policy permits; no complete-history claim if older coverage expires                       |
...
| NPC observations and recallable memories                            | Actor byte/count budgets and game forgetting policy; protected active commitments have separate finite quotas; creator archive permission is separate from recall                                            |
...
This is a one-time explicit importer into a new target, not an in-place silent rewrite. The original save remains intact until an approved operational cutover and backup policy say otherwise. No migration is performed by this design task.
...
These data phases complement the existing [game feature delivery plan](review-and-delivery-plan.md). They are not permission to implement in this documentation task.
...
| D2: durable history and minds     | Independent memories/evidence/conversation/commitments, permanent event contract, checkpoints/retention, ingestion watermarks                                                           | History beyond scrollback; protected promises; actor privacy; consolidation race; archive/recovery drill                                                   |
...
Implement the core read-contract subset alongside D1/D2 so queries are useful early. D3 hardens and exposes the same interface; it must not invent a replacement schema. Build tables only for the feature slice that consumes them, while preserving all key/scoping contracts now. Later anatomy, advanced appraisals, institutions and marketplace features add the reserved families when delivered.
...
Deliberately avoided traps: one global world lock; one table/workspace per NPC; global joins in ordinary gameplay; unrestricted SQL MCP; an EAV row per changing scalar; a complete JSON world rewrite per movement; unbounded permanent tick history; model-generated database schemas; hidden provider-session memory; and treating all-time billing as a lossy daily chart.
