...
Implemented local checkpoint policy (2026-09-26): five minutes of running wall time, three verified rolling autosaves, two verified pre-load recovery packages and separately retained manual saves. Replacement is durable before rotation; invalid points never displace a verified point. These finite retained cuts bound ordinary automatic disk growth without deleting canonical history or external accounting. A 256 MiB package limit bounds each point; interrupted/invalid files may require operator cleanup, so this is not a filesystem quota. The hypothetical hourly/30-day disaster schedule above remains a separate hosted target. Paused/busy behavior and work allowances are owned by [save/load](../../docs/save-and-load.md#current-history-capture-boundary).
...
### Local operational recovery procedure

Stop the server and preserve its original data directory, including SQLite WAL/SHM if shutdown was unclean, or the PostgreSQL database plus the configured local `saves/` directory. Do not copy only a live SQLite main file. Set `AI_BUDGET_USD=0` throughout rehearsal and recovery. Use the same supported build and explicit target configuration; database credentials remain in the operator environment, never in backup payloads.

```sh
AI_BUDGET_USD=0 OPEN_LEGEND_DATA_DIR=/absolute/source pnpm exec tsx scripts/backup-world.ts /absolute/new-backup
AI_BUDGET_USD=0 OPEN_LEGEND_DATA_DIR=/absolute/isolated-target pnpm exec tsx scripts/restore-world.ts /absolute/new-backup
```

For PostgreSQL, also set `OPEN_LEGEND_DATABASE_URL` to the selected source or isolated target. The backup is an exclusively published directory containing `manifest.json`, checksummed gameplay/external-authority streams under `checkpoints/`, and every referenced retained slot under `saves/`. Keep the entire directory together. A missing/corrupt slot, incomplete authority stream, oversized record or unsupported dependency fails the operation. Existing complete outputs are not overwritten. Legacy JSON restore remains bounded to 64 MiB; canonical sources use streamed backups. The importer accepts `SOURCE_SQLITE NEW_BACKUP_PATH` and refuses a nonempty PostgreSQL target; old snapshot sources retain an atomic bounded JSON backup while canonical sources publish this directory format.

Restore to an empty isolated target first and verify world identity, clock, quantities, native progress, RNG, private audiences, definition pins, checkpoint reads and uncertain spending holds before admission. External authority includes accounts, sessions, grants, control generations, exit attempts, historical human ownership and authority receipts/audit. These belong in protected operational backups, never gameplay checkpoints. An existing-world restore preserves its current external authority and forgetting ledger and reapplies current human bindings before installation; a fresh-target disaster restore has only the supplied operational evidence. Reconcile any lost execution interval before re-enabling paid work. Failure during installation leaves the previous database transaction intact and its durable pre-load point usable. A repeated successful load request cannot rewind later progress again.
...
MP01/MP04 record completed local implementation/evidence; hosted readiness remains open.
