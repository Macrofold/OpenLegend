...
Independent player/NPC locks and origin revocation remain authoritative through every continuation. Apply the accepted [NPC enablement policy](../03-design-proposals/invention-governance-and-ownership.md#open-and-locked-invention). A future test must explicitly enable the NPC path through owner policy, use supported families and a separately authorized nonzero cap, and restore the chosen setting afterward; documentation integration changes no deployment setting. Reopening a lock cannot revive a revoked request.
...
Only recipes explicitly compatible with the new owner interfaces, units, state meanings and current laws may finish under pinned versions. Old/new authoritative owners never update the same quantity concurrently. An incompatible process must migrate, finish before activation, or enter an explicit recovery selected under the [accepted risk policy](#accepted-live-change-and-risk-policy). Gameplay risk alone does not prohibit activation; a change without any executable, integrity-preserving transition still waits. Candidate approval and per-world activation are separate records. The receipt identifies old/new manifests, migration digest and effective tick. Larger staged or unloaded-region transitions require additional design and are deferred from this initial policy.
...
## Accepted live-change and risk policy

Accepted target, September 25, 2026; INV-5 owns delivery. This distinguishes an undesirable or uncertain gameplay outcome from an operation the host cannot execute coherently.

- A design revision applies to newly created objects. Existing objects keep their exact design unless explicitly upgraded. Existing work normally finishes with its pinned recipe and requirements where compatible with current shared laws.
- A shared-law change applies consistently to its declared affected scope at a committed boundary. It must not silently leave equivalent objects obeying incompatible laws. Preserve identity, history, possessions, consumed resources and completed progress unless a specifically previewed transition necessarily changes affected gameplay state. Never silently reset a world.
- The World Agent investigates dependencies and proposes one consistent default treatment for each incompatibility class across all affected actors, objects and processes. Use all available evidence; distinguish known effects, plausible risks, unknown consequences and incomplete inspection coverage. An agent's explanation is not proof of complete impact discovery.
- The creator may **proceed despite identified, possible or unknown gameplay risks**, even without a transition considered safe for gameplay. A player with scoped full invention freedom may do so within that authority. Show plain explanations, proposed fallback treatments, affected lists or aggregates with drill-down, evidence/coverage limitations, and explicit acceptance of the exact change plan. Allow revision or cancellation instead.
- Preview and approval bind to exact definition, dependency, world-state and permission revisions. Recheck relevant changes before commit; a materially changed impact or fallback requires renewed approval. Use bounded, paginated investigation with explicit incomplete coverage rather than presenting a truncated list as complete.
- Risk acceptance does not bypass authorization, private-data boundaries, supported host operations, valid references, bounded execution or atomic commits. Unsupported computation cannot become executable by consent. The agent selects/proposes supported transitions; the native mutation owner validates and applies them. No arbitrary AI database edits.

**Removed resource example:** retire availability for new crafting, retain the pinned definitions needed to interpret already-made items, and preserve those items. Identify affected recipes, stocks, assemblies and ongoing jobs. Block new crafting requiring the removed resource. Existing jobs finish only if still compatible; otherwise apply the reviewed suspend/cancel/recovery treatment, accounting for already consumed materials and progress. Do not erase outputs or refund consumed resources merely to make the new schema fit.
