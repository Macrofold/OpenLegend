...
Exact ranking is the initial choice. With approximate pgvector indexes, a `WHERE actor_id = ...` clause alone is not proof that nearest-neighbor candidate generation covered that actor's eligible set: filtering may occur after an approximate scan. Evaluate scoped/partial indexes, suitable partitions or iterative scans against representative selective filters; return coverage and measured retrieval quality, not a claim of exact top N. This also does not license disclosure before filtering. [pgvector filtering documentation](https://github.com/pgvector/pgvector#filtering) describes the relevant behavior.

The requested **20 ms full local retrieval target** covers request admission/queueing, source/required-evidence preparation, database round trips, ranking, result loading and local assembly. It excludes separately reported model/provider calls, including query embeddings. It is unmeasured; neither 1,000 candidates nor top 100 over 5,000 is an established safe cap. Define hardware, dimensions, warm/cold state, per-actor history, concurrent queries and percentile/deadline treatment before selecting a production cap. [D62](../05-project/open-decisions.md#d62--recall-latency-and-search-quality) owns those choices. The two-second generic statement safety timeout below is not this interactive performance target; on timeout return explicit unavailable/partial coverage, never an empty result presented as complete.
