...
Implement typed repository operations consumed by current features during D1/D2; the broader compiler, report service and MCP surface follow D3 in the [production checklist](../../docs/maintainers/production-data.md). Historical reconstruction, replicas and archive readers are conditional capabilities and must report unsupported until delivered. A general query language must not block implementing ordinary memory, inventory or due-process queries.
...
| `actors` / `characters`                 | One world/actor; character view may narrow by supported role | Shared living identity including animals, admitted body/species/capabilities and permitted state; no mandatory human mind or vitals. Existing character consumers may use a scoped compatibility view. |
...
| `relationships`                        | One observer/other entity pair                 | Subjective assessment derived from the owner's knowledge document, evidence and source revision; objective relations remain a separate dataset. |
| `goals` / `plans` / `plan_steps` | One actor-owned intention / plan / next step | Operational status, priority, generation, bindings and actual process/receipt links; proposed steps are not completed actions. |
| `knowledge_documents` | One actor-owned general or subject document | Current text, optional subject binding, revision/tombstone and admitted evidence; distinct from learned executable capabilities. |
...
| `inner_world` | One current world/actor snapshot | Accepted text/revision and publication state; own NPC job or authorized fictional-NPC inspection; human-private notes require their owner's grant |
| `thought_history` | One accepted actor/job presentation entry | Short fictional-NPC reflection thoughts; authorized creator inspection, excluded from actor recall; no human-private content through this dataset |
...
| Authorized world creator/agent | Broad world inspection and private fictional NPC context under an explicit grant; no creator access to human-private messages or private character notes. |
...
### Human-private content boundary

Accepted September 25: creator/god powers do not grant access to human-private messages or private character notes. Private messages have explicit participants; private notes have an owner. Store their audience/ownership separately from fictional observer knowledge and world-editor roles. Public speech or another actor's legitimately perceived evidence follows its disclosed event-time audience; a private human channel must not be turned into NPC evidence merely because it is in the same world.

Enforce this restriction in direct reads, joins, vectors, snippets, counts, reports, MCP, debugging and exports. Derived summaries inherit the relevant restrictions; global statistics must not reveal hidden membership or content. A creator-provided world module or model prompt cannot override access. World export/rewind must preserve protected records through authorized storage operations without making their plaintext available to the creator. Platform operational access, abuse handling and erasure obligations require separately authorized policies; creator status supplies none of those grants. [D48](../05-project/open-decisions.md#invention-governance-controls-and-workshop) retains those remaining product questions.

The selected threat model is game/application access control. A person with direct database or host administration is treated as an infrastructure developer/operator, outside this privacy guarantee; protection against that administrator and end-to-end encryption are not requirements for this foundation. Creator and authorized system-admin save/load rights operate through the game service and do not automatically grant plaintext inspection of private content. Separate audited staff capabilities remain explicit, not a creator-role bypass.
...
Due work uses region/status/due-time indexes; operational goals/plans use owner/status/generation and receipt bindings; knowledge uses owner/subject uniqueness and revision. Keep hot simulation indexes local where per-step geometry or perception requires them. The database should not run one query per actor per tick or prebuild a complete graph of every possible interaction.
...
### Relevance retrieval without whole-history preparation

The target operation accepts the authorized actor/world/timeline, query vector or explicit unavailable status, retrieval purpose and result/byte budget. The database selects current eligible sources and compatible vectors using source-record joins/indexes; callers do not serialize the actor's entire eligible-ID list or format every remembered text first. Fetch selected source text in the same consistent read or verify source revisions before use. Direct required evidence and permitted recent records are merged separately and deduplicated by typed source identity. SQL limits bound returned data, not a claim that every eligible source was indexed or scored.

Missing embeddings return explicit incomplete coverage and preserve required/direct evidence. Pure reads never purchase embeddings. The cognition workflow may separately admit indexing/query-embedding work, whose latency and cost are reported separately. Correction/forgetting and timeline checks apply again before model dispatch or effect admission; a previously permitted search result is not a permanent grant.

Exact ranking provides a reference for quality and a straightforward route for small eligible sets; it is not a permanent algorithm mandate. With approximate pgvector indexes, a `WHERE actor_id = ...` clause alone is not proof that nearest-neighbor candidate generation covered that actor's eligible set: filtering may occur after an approximate scan. Evaluate scoped/partial indexes, suitable partitions or iterative scans against representative selective filters; return coverage and measured retrieval quality, not a claim of exact top N. This also does not license disclosure before filtering. [pgvector filtering documentation](https://github.com/pgvector/pgvector#filtering) describes the relevant behavior.

The owner withdrew the arbitrary 20 ms requirement and delegates architecture, algorithm and performance-budget choices to engineering. Optimize complete gameplay responsiveness, throughput, memory, storage and cost under the [first/growth workloads](data-delivery-and-scale.md#1-what-scaling-means-for-this-product), not an isolated vector timing. Remove unnecessary history scans, text/vector duplication, query fan-out and repeated work before choosing index or worker topology. Keep required fresh evidence on its direct path; optional search may combine structured, text and semantic routes without fabricating completeness or silently changing character knowledge.

Measure admission/queueing, source/required-evidence preparation, database round trips, ranking, result loading and local assembly together. Report model/provider time and cost separately and also show their contribution to the complete interaction. Compare exact/approximate/hybrid quality against representative actor-scoped cases, including corrections and important older events; record chosen budgets, timeout behavior and any relevance tradeoff with the evidence. Do not impose an arbitrary source cap or present any unmeasured latency as a guarantee. The generic statement timeout below is a resource guard, not a responsiveness goal; timeout returns explicit unavailable/partial coverage. These choices no longer need a separate product decision under D62.
