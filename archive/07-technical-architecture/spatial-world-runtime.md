...
Limits are centralized in `SPATIAL_LIMITS`: 128 cells per horizontal axis, 32 surfaces, 128 combined explicit/terrain blockers, 16,384 graph nodes, 32,768 search expansions and 2,048 output points. Their limits are implementation bounds, not measured capacity guarantees. The current graph and shape memoization keys are immutable map identity/revision and relevant body-profile values. Cache construction is synchronous, unpaid native work; there is no worker-readiness race or new simulation clock. Static solids and support patches use cached full-extent AABB trees; split axes follow center variation rather than total slab width. Boolean sight, stance and swept-motion queries stop on the first blocking hit, while ordered acoustic crossings retain one contribution per solid. A support wholly below both endpoints can short-circuit only its top-contact check, never a blocker or an overhead slab. Direct support-ID lookup is constant-time. Exact halfspace checks still decide contact, so a broad-phase miss may only discard a disjoint bound. A required over-limit/invalid world is rejected, never treated as empty space.

Coarse support groups conservatively compare the extrema of plane-height differences over shared integer coordinates. A disjoint group can reject a route before graph construction, and unrelated floors are not baked for that request. The remaining graph still performs exact stance and movement checks. It sorts XZ columns by height, reuses reversible same-surface edges and represents exact-coincident zero-cost seam connectivity with a star rather than a clique. Tolerance-height seams retain their individual directed checks and costs. A shared endpoint permits a direct two-leg support change only after both native segment validations succeed; this can select a shorter valid continuous route than grid search, without changing already-saved routes. Weak connected-component labels reject definitely disconnected endpoints before A*; they do not authorize one-way traversal. Generation-stamped typed arrays reuse graph-local search storage for synchronous, non-reentrant queries, clearing stamps on counter wrap; query history cannot change a later result. Interaction-stance discovery first bounds the relevant support patches and defers exact stance/ray work until candidates are considered. The current maximum of 12 route attempts and coarse grid remain coverage limits; richer stance search and propagation of route budget/status distinctions to gameplay remain SW05 work.
