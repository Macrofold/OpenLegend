...
A static BVH is a measured optimization, not a required new general scene framework. Queries enforce finite candidate/work limits. Overflow is a technical incomplete result: it must not be interpreted as no collision or no occlusion. Mandatory safety queries either use a complete bounded fallback or stop the affected transition visibly.
