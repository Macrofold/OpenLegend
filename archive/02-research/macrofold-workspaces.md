...
## Research recommendation and current application boundary

Give every NPC a persistent, workspace-like mind. Use shared application-owned storage for its memories and state, and bounded reasoning workers for ordinary decisions. The accepted memory design now requires background workspace reflection and published inner-world text; immediate calls remain separate. The original hybrid comparison below explains infrastructure tradeoffs, while broader pooling/hosting still needs measurement.
...
The subsequent [world-state and AI-workflow proposal](../03-design-proposals/macrofold-ai-workflows-and-world-state.md) considers reusable Macrofold additions for typed Jev inference, structured resources and composable jobs. It favors a common AI infrastructure layer while keeping Open Legend's state-transition rules authoritative; physical storage may be provided by Macrofold. The Open Legend adapter now exists; this inspection does not establish deployed support for broader pooling, structured hosting or the accepted file-publication target.
...
The current proposal already uses shared PostgreSQL and asynchronous workers. It already separates episodes, beliefs, relationships, skills and commitments. Before this follow-up it did **not** specify a concrete table layout, indexing strategy or recall evaluation. At that inspection there was no implemented method to replace. The prototype now has bounded structured cognition; the [canonical memory architecture](../../docs/memory-architecture.md) owns its replacement target and [implementation status](../05-project/implementation-status.md) owns current evidence.
...
For the current accepted NPC design, use a persistent scoped workspace for background reflection and dreams; immediate cognition uses compact context without a harness. Hourly small-model consolidation is independent of sleep and needs no sandbox. The server publishes validated workspace prose into one accepted PostgreSQL text snapshot. Fresh model history per NPC job prevents retained transcripts from undoing forgetting. These application requirements supersede the earlier optional-workspace recommendation; the cost/lifecycle observations remain research evidence, not live acceptance. See [memory architecture](../../docs/memory-architecture.md).
