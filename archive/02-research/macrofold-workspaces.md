...
There are concrete application-level candidates to measure. The execution engine defaults to a one-second delay between phase advances, and the Workflow sleeps for that returned delay. Hydration selects four pending objects per pass and transfers them sequentially; each staging call resolves the sandbox/session again. These behaviors can accumulate time independently of VM creation. They are observed code paths, not a measured attribution of the user's delay. [Engine](../../../AgentCloud/packages/core/src/cloud-engine.ts), [Workflow](../../../AgentCloud/apps/web/workflows/run.ts), [provider](../../../AgentCloud/packages/providers/src/vercel.ts).

Prioritize improvements according to measured stage durations:

1. Instrument queue-to-admission, create request-to-ready, restore transfer/verification, harness-ready, first model request/token, first UI event and final durable publication. Compare empty, small-memory and larger continuation workspaces; report p50/p95 plus bytes/file counts and concurrency.
2. Advance ready phases without an unnecessary timer, and batch hydration through bounded parallel reads/writes where safe. Preserve leases, idempotency, integrity verification and provider limits. This is a proposed optimization, not a request to remove recovery guarantees.
