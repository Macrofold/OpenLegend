...
**Open Legend proposal:** store directional relationships rather than one shared friendship score. Alice can trust Ben while Ben distrusts Alice. Separate familiarity, trust, affection, resentment, and commitments, with references to supporting experiences. An NPC's sentence about being angry should be generated from its current appraisal and accessible memory. Other players should discover that account through conversation or observable behavior, as requested in the brief, rather than receiving the actor's private state panel.
