...
For a reviewable draft, select **World agent → Native recipe shortcut → Review in workshop before installing**. The workshop can inspect known recipes and your bound systems, use native validation tools, and prepare a saved proposal; **Apply saved proposal** is separate and does not craft or change existing objects. **What can I build?** reads the supported catalogue without AI. `INVENTION_WORKSHOP_MAX_USD` bounds linked workshop generation (default $5) under the normal spending cap; supplied previews and Apply need no model. The unified owner conversation instead uses the native Macrofold harness with the approved OpenLegend MCP connector. See [scope and limits](docs/architecture.md#invention-workshop-tools).
...
### World-owner relationship inspection

With local god/owner mode enabled, the World Agent panel includes demand-driven definition search, exact source and relationship inspection, current work and retained evidence. This is an out-of-world read view, not character knowledge or a full graph editor. A separately configured, disabled-by-default read-only MCP endpoint uses the same service; see [MCP setup](docs/world-agent-mcp.md#implemented-read-only-bootstrap). No paid model is needed for these readers. The [repertoire foundation](docs/repertoire-foundation.md) and [INV tracker](docs/maintainers/inventions-and-world-evolution.md) distinguish delivered source inspection from future native-harness, interaction, information, joint-work and social-family implementation.

### Unified World Agent authoring

In owner mode, World Agent offers one conversation for investigation and supported authoring. Choose New, explicitly authorize the displayed session ceiling (default $5), then send. Session opening and history reads make no model calls. Configure the writable MCP connection and Macrofold first; the panel explains missing configuration rather than substituting tool-less chat. Exact draft reviews show native coverage and full payloads; Approve and Apply remain distinct from crafting or world resume. Stop this turn cancels only its active run; End session retains history. The native recipe shortcut remains available without the full harness. See [setup and supported scope](docs/world-agent-mcp.md) and [current evidence](docs/verification.md#unified-authoring-interface-and-native-write-journey).
