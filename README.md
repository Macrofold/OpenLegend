...
Open **http://127.0.0.1:3218**. Pause, save through **Game**, advance, then load to inspect same-version restoration. The preset is used only for creation. Existing worlds are updated in place under the development save policy; select a separate directory only to create an independent demo. [Implementation and limits](docs/architecture.md#extensible-attribute-foundation).
