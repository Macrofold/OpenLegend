...
**2026-09-21 — Owner instruction, in force until explicitly removed:** there are no real players and the game is in active development. Do not implement or maintain backward compatibility, legacy readers, old-save migrations or compatibility fixtures when state/models/storage change. Incompatible development saves may be rejected; do not silently load or reset them. Bump the current save-format marker when interpretation changes incompatibly. This policy overrides compatibility/migration requirements elsewhere in the documentation; those are future design context, not current work. Integrity checks, atomic writes, same-version recovery, privacy and real spending protection still apply.
...
After the active development policy is lifted, migrations must be ordered, restart-safe and validated before publication. They operate on a candidate, preserve the original and record their application so retrying cannot apply changes twice. Content migrations must account for changes to relationships and behavior, not just renamed fields. Compatibility has a declared support window; indefinite support for every development format is not implied.
