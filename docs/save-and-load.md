...
Manual saves and portable backups capture native state, durable history, source-version history and optional source annotations. Restore installs these together, preserves present-day forgetting/accounting, and fences source/vector publication with a new generation. Dispatched indexing attempts remain outside gameplay rewind; compatible vectors may be reused, but old-generation completions cannot publish. The importer opens SQLite read-only, keeps an exclusive backup, verifies every copied auxiliary row and the reconstructed gameplay state, and refuses a nonempty PostgreSQL target. Bounded record streaming and an owned serialization worker now support larger checkpoints; [measured scope and limits](verification.md#bounded-history-checkpoints-and-recovery) remain distinct from population or hosted acceptance.
...
Autosaves are server-owned recovery files for the entire shared persistent world, not per-player saves. One scheduler serves the world regardless of which players are connected. Manual save/list/delete/load requires the creator or authorized operator's current save grant; ordinary participants have no save controls or automatic-save request path. Normal gameplay durability comes from database commits. Autosaves accrue only while the world is running; the current presence policy still pauses when all admitted players are absent. A future host that runs without connected players uses the same server-owned checkpoint path.

Gameplay capture flushes preceding native progress, pins one consistent database revision and releases the mutation queue once that snapshot is established. An owned worker streams canonical records, complete cold history, source versions and annotations in stable primary-key pages; it does not hydrate every actor or clone a full world into the worker. SQLite uses a separate WAL reader and PostgreSQL a read-only repeatable-read connection. One capture is admitted at a time; repeated identical save requests share completion, conflicting requests fail busy, and deletion waits for an admitted creation of the same identity.

The physical `records-jsonl-1` encoding carries table coverage, captured revision and SHA-256 integrity metadata; streamed metadata must declare both byte and row counts. Work allowances are 64 rows per database page, 1 MiB per encoded record, 256 MiB per package, two million records and a two-minute scan deadline, with a 256 MiB worker old-generation heap limit. Oversized owners must split growing collections through the canonical codec; exceeding an allowance fails visibly and retains previous checkpoints. Existing compact JSON saves retain their 64 MiB reader guard. This is an encoding change, not a per-feature world version. The exact preceding record layout converts through the existing foundation migration; current packages missing foundation tables are rejected.

Private staging files and metadata are synchronized before directory rename and parent synchronization. A repeated create that finds an existing matching identity verifies the body and synchronizes its parent again before acknowledging success, including after an ambiguous publication failure. Only published complete slots are listed. Startup reclaims a staging directory only when its recorded local process is demonstrably dead; legacy or ambiguous stages require operator review. Catalog reads inspect small metadata and payload sizes without parsing world bodies; public pages retain at most 101 metadata entries and return 100 saves plus a cursor. Directory enumeration remains linear in the manually retained slot count, with asynchronous per-entry work outside the gameplay mutation queue; it never loads save bodies. Catalog permission is checked before reading and again before disclosure. Loads verify complete streams and reconstruct state while play is explicitly paused. This full candidate/install path still needs memory proportional to the saved world; the capture bounds do not promise constant-memory restoration.

Restore preserves current forgetting/accounting and atomically switches the world and its pointer to an already durable pre-load recovery file. Retaining two verified recovery packages protects the previous cut during interrupted installation. Cleanup failure is reported separately from successful installation. Ordinary play releases cold sources again after publication. [Qualification](verification.md#bounded-history-checkpoints-and-recovery) records actual memory and latency, including remaining load and active-scene costs.
