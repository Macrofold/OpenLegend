...
## Reviewed custom attribute binding

The finite `attribute-bindings` authoring kind connects an already admitted custom reservoir/category definition to one existing supported body. This is an owner-authorized instance configuration, not a new law, a character's ordinary ability, or an automatic consequence of definition installation. The World Agent obtains its schema from `ol_schema`, inspects definitions and current body state, saves a draft, validates, prepares an exact review, and applies only after the human approves it. MCP and local application callers use the same authoring dispatcher and native mutation owner.

The payload is `{entityId, attributeIds}` with one to sixteen distinct exact installed attribute IDs. The target is explicit; it need not be the controlled character. Each added attribute starts at its definition's admitted initial value. Existing values, native physiology, senses, controller and active work are unchanged. The adapter invokes `editActorAttributes` with absent-state expectations, rather than writing sparse state directly. An existing binding is rejected instead of silently resetting it. The existing manifest/body limits remain authoritative; no new body model or save version is introduced.

A submitted draft pins its manifest and the selected body's identity. A revision changing the target explicitly captures that new target; merely revising text does not refresh an obsolete same-target pin. Ordinary clock advance or changing reservoir values does not stale an addition. Before approval and Apply, validate the target, definitions, current absence of the selected bindings, current owner/session authority and expected draft revision. The review names the body and exact initial values and reports one affected body. An intervening attachment or definition revision requires refreshed review; the duplicate Apply receipt cannot initialize the values twice.

The existing owner body-edit operation is the authority boundary. Attaching an installed definition is not gated as new invention, just as ordinary use of an admitted capability is distinct from defining it. Creating or changing the definition continues to follow the existing invention policy. This does not decide the broader unresolved owner-authoring exception or permit an NPC to call owner tools.

After activation, native reservoir/category behavior and the existing body-state and concern projections consume the new binding. The relationship reader projects attribute and sense bindings from actual body/manifest state; it does not infer them from labels or generate per-tick graph records. Adding a reservoir does not create a new compatible energy source, a sensor, a replenishment model, or a paid reasoning trigger. Actual autonomous response remains governed by current cognition and its separate qualification.

Removal, arbitrary current-value edits, replacement of native needs, changing the body's sense/controller bundle, bulk species migration and shared-law replacement require their own reviewed native adapters. Do not expand this add-only operation into an untyped property setter. INV-21 owns delivery and remaining work; deferred regression cases belong to the maintainer TODO.
