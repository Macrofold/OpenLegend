...
The exact `POST /api/world-agent/authoring` route and `/mcp` share a 128-KiB byte envelope and strict UTF-8 JSON decoder. Ordinary local mutations keep their 16-KiB envelope; God editor routes retain their existing 1-MiB limit. Route classification uses the parsed pathname, not a substring or query parameter that could broaden access. The transport permits JSON escaping and multibyte intent around a candidate; the shared tool schema still enforces its 24,000-byte candidate limit.

Both adapters reject invalid UTF-8 instead of replacing bytes inside candidate content. They check declared and actual body size, bound body reading to ten seconds, and release request bookkeeping on failure. This does not add transport retries, weaken authentication, or make body completion an approved mutation. See [transport/funding verification](verification/workshop-transport-funding.md) for the exercised local and MCP write cases and remaining qualification.
