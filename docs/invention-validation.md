...
A cached test pass does not replace fresh activation permission, lock/revocation, budget, world identity, or current action prerequisites. A world tick unrelated to the requirement should not invalidate an immutable material-role check. A changed material, owner, source policy, query membership, validator revision, or required scenario can.
