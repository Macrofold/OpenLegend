...
**Status: accepted investigation and optimization design; not a performance fix implemented here.** PF09 owns performance delivery, EPR owns exposure semantics/intake, and SW owns spatial queries. This document narrows the measured first-exposure problem and defines a staged response. [Verification](verification.md#invention-extensibility-review) retains measured evidence; [maintainer TODO](maintainers/TODO.md) owns deferred automated coverage.

## 1. Evidence versus hypotheses

The existing mixed native stress scenario creates many encounter/events and awareness rows during its cold opening. Prior recorded runs failed the requested 3x throughput; timings varied by environment, so cross-run absolute values are not regression evidence. No workshop query change has established that native crowd scaling is solved.

Current source inspection shows two concrete amplification opportunities: `kernel.ts:updateEncounters` emits events for newly perceived entities, and `events.ts:emit` can scan all actors again to determine an external audience. `emit` then inserts one awareness record per audience member through `mutateExperience`; its addition path checks existing source collections. These facts justify measuring repeated listener discovery and growing-history duplicate checks. They do not prove the fraction of CPU each consumes.
...
Reuse phase-local spatial/listener candidates for compatible external events, followed by exact event-time checks. Key reuse by relevant observer, geometry, sense and source revisions. Do not reuse a visibility cache through movement, occlusion or disclosure changes that invalidate it. Private events bypass spatial audience discovery.

Add a domain-owned bounded batch append path for awareness acquisitions when measurement shows repeated scans dominate. Build per-actor source membership once for a transition, preserve encounter/source order, validate all additions, and append without repeatedly scanning the growing collection. Existing receipts, forgetting/revocation and actor identity rules remain in the single semantic mutation owner. Do not add an uncoordinated fast writer in the scheduler.
