...
Status: design for implementation; no optimization or capacity claim is established by this document. [Prioritized tasks and acceptance budgets](maintainers/performance.md) are the execution plan. [Architecture](architecture.md#performance-critical-path) records current behavior; [verification](verification.md#performance-investigation) records the limited measurements.
