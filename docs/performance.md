...
2. The authority validates the intention against loaded state and computes the movement transition. Due simulation work yields at accepted interval boundaries so an enormous CPU batch cannot monopolize request handling.
...
Preserve the accepted [elapsed-time contract](simulation-time.md), ordered consequential outcomes and saved randomness while reducing repeated work. One-game-second integration was an implementation choice, not a clock requirement. The finite native loop compiles sorted actor/animal/heat IDs once per call and reuses them across accepted intervals; status eligibility also includes applicable non-actor entities. Nested plan-command transitions refresh the roster. Future native spawning or component changes must refresh it at their mutation boundary. IDs, not draft entities, survive draft replacement. With no eligible observer, the encounter phase has no work. Finite elapsed integration is implemented under [PF13](maintainers/simulation-time.md); unsupported coupled laws retain the documented fallback. This does not authorize merging independent commands or skipping unmodeled physical/sensory effects.
...
- Finalize bounded work once where the accepted interval semantics permit. Merely replacing a former `advanceWorld(world, 1)` with an unconstrained large call would miss perception, commitments, conversation reconciliation and event boundaries. The implemented interval planner preserves finite known boundaries and explicitly documents sampled perception; richer analytical/local scheduling remains gated.
...
- Maintain reusable indexes for outstanding commitments, recently changed actors and event IDs. In larger worlds add spatial candidate sets and dirty-region invalidation; distance is a candidate filter, not an authorization rule.

Actual game-time throughput must cover the selected clock rate with headroom for commands, persistence and publication. At 1× this is 60 game seconds per real second, not 60 transitions; the number of integration intervals depends on mechanical deadlines and fidelity limits. A fast timer cannot compensate for excessive total work or an oversized atomic interval. Establish an immutable snapshot boundary that lets finalization skip unchanged retained branches; preserve mutable seed/migration workflows, append lineage and privacy invalidation. Measure both initial freezing and steady-state growth, rather than counting only a short pre-frozen run.
...
Keep authoritative hot state bounded by active entities and required working memory. Long-lived events, completed jobs, old receipts and UI scrollback should eventually live in their durable repositories rather than every simulation snapshot. This requires a source-preserving migration, cold lookup for IDs still referenced by obligations/forgetting and explicit restore semantics. A cache eviction is not permission to delete historical truth. Under [D59](../archive/05-project/open-decisions.md#d59--historical-retention-and-command-retry-horizon), ordinary gameplay commands replay full outcomes for 24 hours and then return `expired`; receipt retention uses its implemented durable server-issued epoch/controller-generation boundary. Retained source memory and optional consolidation pressure do not gate native simulation or authorize evidence deletion.

Finite linear needs/work integration is now implemented. Regional idle scheduling and general analytical/coupled systems remain candidates: preserve threshold crossings, event-time awareness, path conflicts, action completion, stable ordering and random choices, or explicitly record an accepted gameplay change. Do not lower fidelity silently to advertise a higher population.

Perception should read position and capability scalars once after movement, query local candidates, compare prior membership with sets and preserve unchanged visibility-array identities. Avoid repeatedly walking draft proxies or copying whole entity subtrees just to compare distances. A cache cannot escape the synchronous phase or input dependencies it proves. Observer-private acquisition is not an externally audible/visible act; external speech and action events keep their own event-time audiences. This does not skip commitments or merge distinct events. The [sound-branch coordination note](maintainers/speech-time-integration.md) identifies the additional frozen exposure-phase proof that must be reconciled per accepted interval.
...
Buffers have byte/count/age limits and a defined overflow policy. Optional pending trace snapshots can be replaced by newer versions and report capture gaps. Visual updates can be recomputed from a compatible baseline. Accepted world events, source revocations, command receipts and spending records cannot be dropped. On actual storage failure, stop authoritative admission and expose degraded/paused state. Optional memory consolidation pressure is not that failure and must not stop walking. Queues smooth bursts, not sustained overload.
...
Action acceptance and first visible movement are different milestones. The current 50 ms host wake improves on the former 250 ms opportunity, but does not require a fixed amount of native work or a frame per game second. Retain independent one-second routine durability and coalesced optional publications. Measure movement latency against timer/projection overhead; client prediction remains the separate network-latency option. State delivery is not a rendering-frame limit.
...
This is the accepted optimization direction, not a declaration that every dense 8x workload has passed. PF12 sequences the broader PF/SW/EPR work; [PF13](maintainers/simulation-time.md) owns the finite elapsed-time delivery. [Earlier rebase evidence](verification/spatial-rebase-eightx.md) records the former per-second baseline; [current cadence evidence](verification/simulation-cadence.md) records actual elapsed progress and its limits.

### Define the rate before optimizing

The current host has a base ratio of 60 game seconds per real second: 8x requests **480 game seconds per real second**, not eight. This is an elapsed-time target, not 480 required ticks. The former 2.08 ms-per-tick arithmetic does not impose an integration cadence. Measure total CPU per game-time span, actual interval counts and command tails, leaving headroom for durable boundaries, publication and GC. Retain the approximately 8 ms cooperative batch budget; one oversized atomic interval can still exceed it. Short samples do not prove a tail-latency guarantee.
...
Use the starter scene, the 318-entity sensing workload and the earlier combined 329-entity/23-surface workload as reproducible cases, not population limits or a promise that every spatial distribution passes. Independently vary history, observer count, moving targets, sounds, profile count, stacked floors, render coverage and geometry edits. Short current-server results do not qualify the combined sound branch, long sessions or every dense distribution.
...
Cache **dependency-valid results**, not just individual ray intersections. A stationary observer looking at unchanged objects and geometry should not reconstruct the same static visible list every interval. Keep static object membership/poses separate from moving actor targets. Geometry, source identity/pose/body, observer pose/anchors/range/capability and detector policy are explicit invalidators; metadata/recognition remains freshly observer-scoped rather than cached with geometric visibility.

Use dirty entity/region work from the existing mutation owner and a phase-local scalar view, not a full-world deep copy into each worker. Snapshot/fork/load boundaries must not share mutable or revoked draft references. Start with unchanged static objects, then incremental cell membership. Moving entities update only affected neighborhoods. Preserve deterministic candidate/event order; unchanged arrays can be retained, while changed membership retains its acquisition/loss semantics.
...
The current thresholded transmission query is implemented: an admitted attenuation factor below the requested minimum proves rejection immediately, without constructing/sorting remaining crossings. Successful numeric answers and full-transmission callers retain canonical multiplication order. This relies on the finite family's factors being in [0,1]; amplifying authored media require new valid bounds. Do not prune a marginal multi-barrier product by a differently ordered floating-point approximation. The parallel sound branch's graded dB implementation retains its own validated candidate/transfer rules rather than copying this older binary threshold policy.

Reuse geometry-only acoustic transfer for identical emission/listener anchors and relevant geometry versions, separately applying event semantics and listener thresholds. Retain historical emission origins. Batch sounds sharing the same stable sensing boundary, not merely sounds arriving in the same wall-time interval. Full/muffled detection, localization and word/identity disclosure remain distinct EPR responsibilities. Playback mixing can be budgeted independently; the awareness ledger cannot lose events because the audio mixer is full.

### 3. Keep continuous travel cheap after a route is known

Retain Recast and the exact direct-path shortcut. Prewarm common profiles and reuse compatible prepared data; no per-actor WASM instance. Coalesce identical derived requests only when geometry, body envelope, support endpoints, traversal policy and knowledge scope all match. Keep the bounded worker/queue, deterministic admission and stale-result fences.
...
The host already has bounded routine persistence, frozen snapshots, append-aware history, bounded yielding and trigger-based background work. Reuse them rather than adding a second journal or universal queue. Conservative status participation and finite elapsed rates are implemented; richer applicability/deadline indexes remain candidates. Indefinite sleep/needs/wake effects still advance semantically.

Move genuinely cold historical data off repeated finalization/serialization paths under PF08 while retaining evidence, stable IDs, pending commitments, current knowledge and explicit durable reads. The [boundary catalogue](maintainers/simulation-boundaries.md) distinguishes integrated finite laws from general net-flow coupling, local schedules and analytical dormant worlds. An interval cannot skip an unmodeled interaction merely because a formula produces a plausible final number.

### 5. Separate simulation, replication and rendering rates

Simulation speed is not display refresh rate. Use existing publication owners to coalesce replacement-only poses to a measured real-time cadence (20 Hz is a starting experiment), while camera/animation render at device cadence, normally targeting 60 FPS or display refresh. Action acknowledgement, removals, authorization revocation and ordered story/speech/effect records must not wait for a pose throttle. Slow clients resume from authoritative scoped state plus reliable history, not a backlog of every obsolete pose. Never send an NPC's future path to make interpolation easy.

Measure browser/SSE/React work separately from the server native budget. If optimized aggregate work still monopolizes the host, evaluate one long-lived simulation owner worker receiving commands and sending permitted deltas, not full-world copies every tick. This is PF10's gated ownership change, not an additional competing world writer. Async syntax does not offload CPU.
