...
The initial embedding index uses OpenAI `text-embedding-3-small`, 512 dimensions and cosine ranking inside PostgreSQL using pgvector over actor-scoped revision-compatible rows. Search returns at most 24 IDs/scores; mandatory evidence has an independent path. Incremental indexing handles 32 changed sources per call, query caches retain 16 embeddings, and vectors persist until their source or model becomes invalid rather than being evicted at 1,024 entries. Old JSON caches migrate transactionally inside PostgreSQL without paid re-embedding. SQLite reports semantic retrieval unavailable and preserves structured/mandatory evidence. Raw recall is six hours; backlog 8,192 records; summaries 256 with thirty-day routine expiry and protected high-salience retention; thought history 100; diagnostics 1,000 roots/stages combined. Overflow is visible rather than silently losing protected evidence. Remaining work is held-out quality/cost tuning, comprehensive recovery/fixture acceptance and any later repeat-dream or demonstrated-omission recall-tool requirement.
