...
Autonomous cognition consumes a current opportunity snapshot, not batches of old unprocessed events. Its wakeup watermark covers all observed experience at admission (including native handling and failed decisions); the newest eight significant changes summarize the trigger. Retained older evidence remains queryable through the hard-query and semantic paths. It never independently forces backlog-draining calls. New evidence arriving after the snapshot, changed goals, interests or meaningful state can create a fresh opportunity under cooldown/spending admission. Reflection/consolidation scheduling and retention remain separate.
...
| Current conversation         | Newest 32 personally experienced messages in the active conversation, presented chronologically, on any cognition trigger          |
...
Optional sections use scoped semantic top-300 retrieval when larger than 300; smaller sections go directly to Jev. Older current-conversation speech follows the same optional retrieval policy. The action library comprises all actor-permitted native and planning candidates. Above 300, index the complete library and retrieve semantic top 300; at or below 300, retain every candidate without purchasing unnecessary embeddings. An unavailable action index uses stable-ID top 300 with explicit incomplete-coverage diagnostics. Relevance only filters suggestions; it never selects or authorizes execution. Unlisted proposals remain available through the existing response contract.

Jev requests have no application question-count cap. Fit actual serialized state and questions under the conservative estimated token budgets in [provider limits](ai-providers.md#provider-behavior-and-limits), recording size omissions. The generative model retains its existing 100,000-byte complete-context allowance; fit action descriptions within the remaining space and report omissions. Required evidence survives optional attention failure. Retrieval admission, batch transport sizes and retention windows are separate policies; increasing question capacity does not authorize unbounded world/history disclosure or automatic paid retries.
...
The production schema uses actor-scoped awareness and memory records alongside `mind.inner_world`; see the [data model](../archive/07-technical-architecture/production-data-model.md#8-minds-evidence-knowledge-and-conversation). Storage changes follow the [active development policy](save-and-load.md#active-development-policy): reject incompatible development saves, preserve same-version integrity, and add no old-format conversion. This plan does not imply those tables exist today.
...
At every simulated-hour boundary, schedule bounded cleanup of routine raw personal memories and aware world events **older than six in-game hours**. Preserve the newest 512 speech experiences per actor as verbatim sources for current-conversation retrieval; older speech becomes eligible for consolidation as newer speech displaces it. This count bound avoids tying conversational continuity to accelerated simulation time while keeping storage finite. Other recent raw context uses the six-hour window. A delayed older backlog is not dumped raw into the next conversation.
...
Keep separate finite limits for raw experience, the newest-512 verbatim speech pool, consolidated memory, authored inner-world text and god-only history. The six-hour routine window does not imply unlimited rows during a busy hour, and consolidated summaries do not imply unlimited lifetime retention. Tune record and byte budgets together. Native exact deduplication and supported decay work without a model; semantic summarization needs its separately admitted call.
