...
An in-flight response refreshes only when that actor receives a newer awareness entry meeting both configured thresholds. The initial thresholds are importance **8** and urgency **8** on the 0–10 scales. The director aborts the active provider stage promptly; the interrupting event becomes required evidence and provider/application stage identities distinguish the refreshed attempt. Refresh at most once: a second qualifying event does not create an unbounded paid retry loop. Native emergencies still act immediately, and deterministic response admission independently rechecks action targets, range, inventory, costs and plan prerequisites. Importance alone continues to govern notability/retention; urgency alone grants neither retention nor disclosure.
...
The right-side Intelligence panel is a development inspector grouped by semantic trigger/action, rather than requiring application-log searches. Selecting a trigger replaces the list with a full-width detail view and standard back control. The React view renders common trigger, response, Jev, speech and embedding facts directly. Persistent JSON icons open the complete request or stage record—including captured provider exchanges—in a separate JSON panel, instead of presenting transport exchanges as numbered peer requests. Context/retrieval and background-job records appear as their stages are captured. Fixture coverage verifies the inspection flow without inference; live-provider quality and recovery remain separate acceptance work.
