...
World state, remembered experience, authored inner world and thought presentation are separate. Inventory, health and location belong to the simulation. Raw/consolidated experiences supply recall. Reflection edits the inner world. God-only reflection thought history remains presentation, not another recall store. Under the future composed-response contract, an accepted intentional `think` is instead an actor-owned raw experience with `self_thought` acquisition, source component identity and entity targets. It is recallable and consolidatable, never a world event or proof of the thought's subject. Player-private Narrator prose is a separate derived presentation and does not become actor memory.
...
A simple conversation may need only speech. The accepted [talk/act/think contract](narration-and-conversations.md#3-talk-act-and-think-response-contract) permits optional speech, a known action handle or bounded effect-free expression, and a brief private in-character thought, in any combination for any admitted trigger. None is mandatory; an explicit no-response is valid. The current immediate runtime uses one strict envelope with nullable talk/act/think and a flat total action object compatible with OpenAI strict structured output. Context uses labeled English sections and durable event-time trigger attribution. For speech, the route judgment includes a conservative preliminary gate for whether action context might plausibly help; only a confident negative skips action retrieval. An open gate refreshes native-feasible candidates and filters them in one separate Jev batch. Failure of that optional batch exposes the bounded candidates rather than failing an otherwise valid response. Stored memory acquisition labels remain evidence, not a requirement to recite provenance in spoken dialogue. Ordinary responses still exclude `policy`, `expectedRevision`, `documents`, `removeDocuments`, `records` and mind-patch arrays. The optional `think` is fictional authored experience, never hidden provider reasoning or a required psychological checklist.

The server binds the result to the admitted job, validates its bounded schema and rechecks current action prerequisites before committing effects. Native memory/awareness ingestion proceeds whether or not an LLM runs. Immediate response components have independent outcomes: accepted speech/action enter self-awareness once, accepted thoughts enter personal experience once, and failed proposals are not remembered as completed deeds. Aggregate response status distinguishes full, partial and total rejection so accepted components are not falsely presented as an all-or-nothing success. The newest 300 response receipts form a saved operational replay/debug window; memory and later history retrieval have independent retention. Dialogue, immediate action, private thought and background reflection have distinct commit boundaries; there is no mandatory highest-tier bundle holding them hostage.
...
The personal-perspective migration replaces the local character's legacy default names (`You` or `Player`) with Mike and rewrites supported native narration in existing personal stores. Preserve event ownership, IDs, source attribution and quoted speech; update changed summary/inner-world revisions so derived context can refresh. Do not invent provenance for uncertain legacy prose. The implemented wording conversion is rule-based, and consolidation/reflection instructions carry the same policy; arbitrary legacy prose and model compliance still require acceptance. Isolated native execution confirmed gathering perspectives, attributed speech, repeated migration and inspector source tags. Existing running-save/restart verification, comprehensive automated coverage and live model acceptance remain pending in the [maintainer TODO](maintainers/cognition-redesign.md#migrated-acceptance-gaps).
