...
The current spatial implementation does not become fully 3D by changing an asset or adding a height field. New inventions bind actual supported spatial/query interfaces. Neither Euclidean ground-plane assumptions nor omniscient visual access should be embedded in generic invention coordination.
...
Publication rechecks candidate identity, selected revision, relevant dependency/constitution versions, current rights and lock watermarks, budget eligibility, and world/authority generation. Mechanical activation, visual manifest publication, instance creation, and accounting settlement each keep their appropriate independent atomic boundaries. Do not hold a database transaction open while a provider or reviewer is working.
