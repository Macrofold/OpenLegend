...
Art and mechanics have independent artifacts, reviews, and publication receipts, connected by explicit compatibility. An image cannot add anatomy, extend reach, alter collision, create resources, light a fire, or turn an attempted action into a committed outcome. Faithfulness is relative to the admitted observation model: an authorized illusion/disguise may conceal physical truth, but its appearance cannot itself install deception mechanics. Improving art cannot silently redesign the chosen invention.
...
Bind visual adequacy to the [spatial world's](spatial-world.md) supported body, surfaces, anchors, and camera views. A single fixed-angle sprite cannot claim correct rear or overhead geometry under unrestricted orbit. Use supported directional frames, a view-valid fallback, or an explicitly selected sprite-friendly presentation policy; do not silently lock the camera or generate paid art on every camera movement. A new mesh's visual extents do not change collision, support or traversal.
...
Define the minimum representation per supported family/state. A neutral icon can represent an inventory-only tool. A trap needs readable presence/footprint and required cues for observers entitled to perceive them; an undiscovered trap must not leak its identity through a fallback. A building opening must agree with traversal/occlusion. A creature needs enough pose/state information to avoid showing a living animal where only remains exist.
...
[Observer-relative presentation](repertoire-foundation.md#7-observer-relative-presentation) owns the cross-system illusion/disguise and audience distinction; [information artifacts](repertoire-foundation.md#5-information-artifacts) separates represented content from carrier and generated bytes. These contracts do not claim those world families are implemented.
