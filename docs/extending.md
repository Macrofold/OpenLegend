...
Event-time awareness includes the player. Retain one shared witnessed event plus permitted actor projections, not duplicate raw memories; unwitnessed actions still commit native state. Consolidate older-than-six-hour experience hourly using a small-model route, independently of reflection and sleep. Publish validated reflection files atomically as one current PostgreSQL text row, preserving concurrent observations and the old snapshot on failure.
