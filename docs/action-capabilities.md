...
Use exact complete binding without inference first. Otherwise use Jev for finite full-match selection/classification before generative parameterization. A clear matching handle needs no generative translation. A bounded generative stage supplies required parameters and the omission report; Jev can classify whether a proposed omission is tolerable, needs the initiator's decision, or invalidates the method. Initial required winning-choice probability is 0.8; uncertainty chooses interpretation or confirmation, not automatic execution. A model's semantic judgment never overrides native validation, scope, costs, or authority.
