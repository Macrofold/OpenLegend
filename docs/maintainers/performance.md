...
- [x] Implement bounded already-due [native execution slices](../performance.md#native-execution-slices), with one-second physical/logical boundaries and early exit on meaningful occurrences/restrictions. The reference path remains available; differential, cold and sustained qualification continues below rather than being implied by batching.
