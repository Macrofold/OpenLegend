...
| `execution`         | `single-step` (default) calls the one-second reference path; `native-slice` uses the server's bounded already-due snapshot slices. `steps` and `warmup` remain simulated seconds, not invocation counts. |
