...
- [ ] Update existing response fixtures from `plan.actionIds` to total `plan.steps`, and save fixtures to schema 8 / `development-2026-09-22-plan-outputs1`. Automate strict provider encoding, backward-only references, missing/consumed/foreign outputs, merged stacks, chained cooking, restored running work, interruption and no continuation inference.
