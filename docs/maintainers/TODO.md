...
- [ ] **SR02 — Spatial route integration:** preserve stacked/fractional endpoints, native duration/consumption and support identity across direct routes, Recast corridors and failed preparation. Current detailed Recast/body cases are SR16–SR17; do not require deleted lattice node order, closed-region caches or native A\* heuristics.
...
- [ ] **SR10 — Observer-private acquisition and introductions:** cover separate observers acquiring the same source, sleepers/dead/non-memory actors, private contacts interleaved with visual entries, reentry, same-version restore, removal and source/sense changes. Only the visual observer receives the acquisition; seeing the observer does not reveal its hidden target. Preserve native living-entry reconsideration and the observer's designated-story milestone, while rejecting third-party/private-thought/disabled-policy narration and avoiding automatic stories for ordinary objects. Ordinary speech/actions still use current event-time audiences. Compare each observer's own valid entries, not the obsolete external acquisition audience or its removed cache.
...
### Recast and presentation regression TODOs

The old SR09/SR11/SR13/SR14 lattice-specific tests are superseded by SR16–SR17; their meaningful support, clearance, lifecycle and privacy cases are preserved below. Existing tests expecting old RouteResult/synchronous search or uninitialized collision creation must be adapted, not counted green or deleted to hide a failure. Current evidence is [manual/build observation](../verification/recast-integration.md).

- [ ] **SR16 — Shared body and corridor contract:** initialize pinned runtimes, derive capsule/cylinder radius/height/skin and Recast voxel units, exercise direct/arbitrary-angle routes, diagonal/fractional corners, 80 cm passage versus larger body, low lintel versus hare, ramps/crests, overlapping/identical/different floors, exact named endpoint and bounded string-pulling. Every simplified segment requires a full sweep; raster height correction may not change storey or create steps. Compare accepted motion with an independent physical reference. Cover start penetration, tangent/skin contact, slab underside, dead-body cylinder, body/profile edits and zero-distance seams. No requirement for old A\* node numbers or four-direction paths survives.
- [ ] **SR17 — Required-data scheduling and recovery:** slow/failed build, query budget/partial/unsafe/unavailable, queue >64, >6 profiles, 20-second watchdog, action cancel/replacement/death, position/map/profile edit, manual pause, restore/restart and shutdown. Verify no transaction held for worker CPU, no game-time/hunger charged for valid required-data wait, existing debt retained, no busy catch-up loop, stale callback cannot clear a new active task, obsolete worker terminates before replacement, no unsafe result/retry effect and no paid inference. A saved pending request rebuilds derived state; raw WASM/polygon handles never persist. Compare fast/slow worker runs at equal accepted simulation time.
- [ ] **SR18 — Presentation authority and settings:** off/player/nearby, radius and strength validation/persistence, immediate knowledge revocation, ghost exclusion, unknown source/floor leakage, eligible target behind an out-of-radius tree and partial screen clipping. Verify only currently authorized hidden target fragments redraw, feather/stipple/color compose with ordinary depth, and picking uses the same alpha/feather scope. Cover overlapping reveal targets, large transparent regions, no depth-writing overlay, no hidden background hole, generated sprite reuse/release and world/load reset.
- [ ] **SR19 — Lighting, shadows and motion:** ordinary unprepared images receive ambient/local lights while emissive effects remain intentional; camera yaw/pitch does not introduce false anatomy or unstable proxy shadows. Follow ramp ascent/descent and bird flight with shadow footprints spanning receivers, preserve occlusion under floor cutaways, tune bias at low sun/thin slabs, confirm observed-light cap/source revocation. Exercise right-click versus orbit drag, smooth pitch/lock/pan/projection, settings/accessibility, support-aware interpolation and large missing-motion fallback. Capture actual browser/GPU behavior rather than assuming a compiled shader is correct.
- [ ] **SR20 — Sustained full-stack load:** automate the documented manual native/worker/HTTP/save observations under supported high-density geometry and populations, then soak repeated rebuild/load/cancel and large rendering coverage. Measure end-to-end percentiles, responsiveness, retained JS/WASM/GPU memory and disposal. Separate cold build, first exposure, steady-state movement, exact route validation, durable storage and rendering; no component-only 90% claim. Production PostgreSQL and multiple clients/worlds remain distinct qualifications.
...
- [ ] Update existing response/goal/save fixtures for the current in-place spatial model updates, then add automated coverage for the [AG11 matrix](agent-agency.md#ag11--integrated-deterministic-and-adversarial-acceptance). Cover strict provider encoding, repeated operations, partial admission, alias/reference attacks, actual durations/material conservation, saved running work, cancellation, terminal-history bounds, reflection revisions, scope/privacy, starvation adequacy, private feedback and stale/retired job admission. Use the current startup/load update owner; do not require a save reset or add a parallel legacy runtime.
...
## Mixed spatial review — additional qualification

The [mixed review evidence](../verification/mixed-spatial-review.md) records builds and actual runtime/browser observations, not completed automated regression gates.

- [ ] **SR21 — Full-facing sprite and depth equivalence:** sample yaw/pitch/projection after an actual rendered frame; camera matrices can lag a command until rendering. Compare visible image aspect, feathered reveal, virtual-body depth, alpha picking, status anchors and world-stable shadow orientation. Include camera-close and near-plane cases, negative scale/facing, bob/animation, cutout versus blended/remembered art and overlapping eligible targets. Shader compilation alone is not visual correctness; neither may camera rotation change physical collision or bodily sensing.
- [ ] **SR22 — Shadow/reveal resource lifetime and cost:** assert shadow-only proxies never enter color/depth color-pass lists, including hierarchy toggles, floor focus and observed-to-remembered transitions. Remove all instances/rebuild repeatedly with retained shared proxy geometry; verify balanced mesh/material ownership and correct shadows after recreation. Exercise many receivers/lights and all reveal modes, texture replacement/disposal and zero-strength/revocation paths. Measure actual GPU/pass work; draw-count reduction is not an FPS guarantee.
- [ ] **SR23 — Spatial query/worker boundaries:** compare indexed corridor candidates against complete support enumeration, including stacked and partly overlapping ramps and tolerance edges. Compare terrain-cell supercover against independent crossing calculations for zero/short/reversed/diagonal paths and grid boundaries; do not require position quantization. Exercise dispatch failure, clean worker exit, concurrent store failure/publication/retirement/close, stale load results and bounded retry behavior. Qualify source admission and native time preservation without replacing the existing world writer.

## Spatial rebase and eight-times qualification

No automated tests were written or run for this pass; [manual runtime evidence](../verification/spatial-rebase-eightx.md) is not regression coverage. PF12/SW/EPR own feature tasks.

- [ ] **SR24 — Main/spatial integration:** exercise in-place current-state upgrade and manual restore with pending/completed Recast routes, asleep/stunned perception gates, horizontal status art and virtual picking/depth, item piles/pickup/strikes, follow camera, source-scoped identity and own-observer introductions. Configuration and native profiling entrypoints initialize their actual geometry dependency. A blocked native profile must report zero actual progress, not a requested-step capacity pass.
