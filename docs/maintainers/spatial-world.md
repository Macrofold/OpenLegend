...
The current provider is pinned Recast/Detour in one reused server worker with selective Rapier shape queries. The former dependency-free lattice planner has been removed. [Architecture](../architecture.md#spatial-world-foundation) owns current facts; [runtime](../../archive/07-technical-architecture/spatial-world-runtime.md) and [presentation](../world-presentation.md) own accepted behavior; [current observations](../verification/recast-integration.md) are not automated acceptance. SW17–SW19 consolidate the agreed movement/presentation cutover without replacing still-valid SW01–SW16 requirements.
...
| SW06       | One reusable bounded worker, required-data time barrier, saved pending routes, no-inference continuation           | Multi-world fairness, bounded multiworker scheduling and production worker-failure/soak qualification              |
| SW07       | Current static sweeps, bounded replans, conservative support removal rejection and bird fall/landing               | General crowd avoidance/fairness, dynamic doors, structural support loss and fall consequences                     |
| SW08       | 3D bodily sight/hearing, range-first sound, ordered attenuation, cache invalidation and route privacy              | Richer EPR03 exposure deltas, graded unknown/muffled sound contacts and genuine first-exposure scaling             |
| SW09       | Continuous right-drag orbit/pitch, left/middle pan, zoom/projection/floor/lock and local preferences               | Server viewport leases/multi-view D51 and broad touch/accessibility qualification                                  |
| SW10       | WorldRenderer seam, camera-facing lit sprites/virtual depth, mesh art and support-aware interpolation              | Minimal RenderWorldView, directional/3D rigs, late model loading and full representation swapping                  |
| SW11       | Actual-surface picking, cutaways retaining physical/acoustic/shadow geometry, scoped read-through picking          | Broad overlapping-target/alpha/depth/low-end GPU and restore qualification                                         |
| SW12       | One saved native bird corridor with body sweeps and indexed landing                                                | Generated air graph, alternate perch/fairness, unrestricted flight or full loss-of-flight dynamics                 |
| SW13       | Fallback generated imagery remains usable and lit without altering mechanics                                       | Generic spatial invention, validated late normal/depth/model generation, permissions and asset epochs              |
| SW14       | Queue/build/query metrics; actual native/worker stress and running-app observations                                | Sustained CPU/heap/WASM/GPU/tail latency, inspector overlays and many-world/client tests                           |
| SW15       | Schema-10 pending/accepted route, support and flight state; rebuild derived runtimes, saved reveal profile         | Crash/rollback/late callback/restore automation and future dynamic-physics state                                   |
| SW16       | Earlier native/browser evidence plus current compile/native/HTTP observation                                       | No new automated suite run; SR16–SR20 and broader matrix remain open                                               |
...
- [x] **SW05.1a (superseded provider)** Historical lattice optimization; current Recast replacement is SW17. Preserve its earlier performance evidence in Verification, not as a second active provider requirement.
...
- [ ] **SW06.2a** Qualify cold-build and queue tail latency under genuinely dense distinct geometry, many profile changes and simultaneous worlds before increasing admission limits. Off-thread Recast prevents main-thread construction stalls but does not eliminate seconds of cold required-data waiting. Consider affected-tile updates or more workers only after attribution, retaining time/epoch/authority boundaries.
- [ ] **SW08.4a** Complete EPR02/EPR03/EPR10 and PF qualification for concentrated first exposure. Initial visual acquisition is now observer-private and designated self introductions are preserved. Every genuine acquisition/recipient still requires storage; external-event candidate indexing, exposure deltas and long-history persistence remain open. Qualify large memory-capable populations and all-to-all hotspots without dropping evidence or weakening physical safety.
...
- [x] **SW05.1b (superseded provider)** Historical lattice optimization; current Recast replacement is SW17. Preserve its earlier performance evidence in Verification, not as a second active provider requirement.
- [x] **SW08.4b** Maintain source-ID membership inside the central experience mutation owner's draft. Initial acquisition now uses the observer-private EPR03 path rather than the superseded encounter-audience cache. Preserve owner/forgotten checks and invalidation; richer incremental sensing remains open. See SR08/SR10 for deferred regression coverage.

- [x] **SW05.1c (superseded provider)** Historical lattice optimization; current Recast replacement is SW17. Preserve its earlier performance evidence in Verification, not as a second active provider requirement.
...
- [x] **SW05.2** Implement surface-bound start/destination projection and finite interaction-stance search. Reject wrong-floor projection, overlarge snaps, inaccessible target centers, and unsupported native links. The current Recast adapter has sampled runtime evidence; its new automated qualification is SR16–SR17, not the historical lattice fixtures.
...
- [x] **SW06.2** Direct clearance remains synchronous; nontrivial queries and preparation use saved required-data requests outside the writer, then revalidate at the native boundary. No synchronous expensive fallback planner remains.
...
- [x] **SW06.4** Make technical preparation independent of simulated need/action duration. Preserve simulation debt and show a technical preparation state at a required blocked boundary; never manufacture in-world waiting based on worker speed or drop elapsed simulation work silently.
...
- [x] **SW17.4** Revalidate accepted routes and actual moving displacements, bound replans, preserve native action consumption/timing and arrival tolerance. Show preparing status and keep reads/cancel/pause responsive.
- [x] **SW17.5** Calibrate current radius/skin/voxel/contour values with actual ramp, narrow passage, lintel and overlap probes; 8 cm XZ and 5 cm Y raster with separate physical support tolerance. This is sampled evidence, not universal collision coverage.
- [ ] **SW17.6** Integrate Rapier kinematic sliding/ground snapping/steps only after a shared support-aware controller policy is exercised. Current routes stop/replan rather than a second dynamics controller moving actors. Teleport execution is not a prerequisite; itinerary routing and low-cost-link heuristics remain separate.
- [ ] **SW17.7** Add real affected-tile updates, runtime traversal links/permissions/hazards and profile-aware cache scheduling when supported authored geometry needs them. Reject unsupported structural edits meanwhile; do not keep navcat or the old lattice planner as parallel production backends.

- [x] **SW17.8** Bound worker failure/recovery and drain publication/retirement before store shutdown, including clean unexpected exit and synchronous dispatch failure. A failed commit uses the existing storage pause rather than a recomputation loop.
- [x] **SW17.9** Limit physical corridor projection to indexed nearby supports and replace terrain point sampling with exact crossed-cell traversal. Preserve body sweeps, ordered acoustics, support identity and fractional endpoints.

## SW18 — World presentation delivery

[World presentation](../world-presentation.md) owns the behavior; these are implementation states, not competing shader specifications.

- [x] **SW18.1** Right-drag continuous yaw/pitch, left/middle pan, stationary context click, optional lock and gesture cancellation. No provider work on camera movement.
- [x] **SW18.2** Fully camera-facing sprites preserve image proportions under yaw/pitch, with upright virtual depth shared by drawing, lighting and alpha/depth picking. Keep cutout/dither and emissive/blended effects separate. Support-aware visual interpolation prevents a descending ramp from snapping prematurely onto underlying ground.
- [x] **SW18.3** Lit-by-default sprite approximation, rounded normals/wrapped diffuse, ambient/sun and bounded observed fire lights with restrained post-processing. Preserve existing generated assets as fallback; no art-generation prerequisite.
- [x] **SW18.4** Replace centroid-selected blob shadows with true receiver projection using simple world-anchored shadow-only bodies/canopies. Ordinary mesh shadows remain; floor cutaways retain shadow geometry.
- [x] **SW18.5** Save profile-scoped reveal off/player/nearby, radius and strength. Render only hidden authorized target fragments with feather/stipple; revoke immediately, preserve alpha/depth-aware picking and exclude stale ghosts. No target-by-occluder CPU loop or whole-screen X-ray hole.
- [ ] **SW18.6** Qualify visual matrix across rotated/low/high cameras, stacked occluders, ramp shadow split, canopy silhouettes, screen edges, lit backside, night/fire, settings reload and overlapping eligible targets. Include low-resolution dither versus TAA, devices/accessibility and low-end fallbacks; compilation is not art acceptance.
- [ ] **SW18.7** Improve directional/normal/depth assets, foliage translucency, shadow silhouettes and optional ambient/environment lighting after art review. No claim of Octopath/Backbone/Project Zomboid parity from the current fallback. Consider clipped blob shadows only as a measured quality fallback, not a second mandatory system.
- [ ] **SW18.8** For long network gaps, consume authorized recent-past motion and smooth turns without disclosing NPC future paths or cutting corners. Current unsafe topology interpolation snaps to known state. Complete renderer/input extraction and minimal RenderWorldView when another consumer requires it.

- [x] **SW18.9** Keep shadow-only instances out of color lists, share a retained lightweight proxy mesh, batch plank detail per surface and reuse reveal uniform storage. Skipped work still processes immediate authorization revocation and actual input clicks.
- [ ] **SW18.10** Qualify retained layer/resource counts and framebuffer/depth agreement under close perspective, near-plane crossings, overlapping revealed targets and repeated camera/load changes. Investigate instancing/batching for many billboard/proxy instances and device quality budgets using actual GPU traces before adding another rendering system. Mixed software-rendering evidence does not qualify physical-GPU FPS.

## SW19 — Full-stack scaling gates

- [x] **SW19.1** Run bounded native population, distinct/stacked geometry, cold/warm worker queries and main-thread responsiveness observations without automated test suites or model calls; record actual counts, hardware and limitations.
- [x] **SW19.5** Exercise a combined real application scene with added memory-capable actors, native flyers, objects, stacked supports, fire lights, moving requests, persistence and camera orbit. Record complete workload/runtime limitations in [mixed review evidence](../verification/mixed-spatial-review.md); no universal population or frame-rate claim follows.
- [ ] **SW19.2** Attribute command-to-display tail latency across queue/native support validation/finalization/SQL/projection/transport/GPU before claiming another overall percentage reduction. Do not confuse raw Detour query cost with complete accepted route cost.
- [ ] **SW19.3** Probe many distinct objects/surfaces, moving flyers/overlapping bodies, many profile meshes, repeated load/rebuild/failures, retained WASM heap, high-coverage read-through, many shadow casters and local lights, camera rotation allocations, picking and CPU/GPU stalls. Derive concurrency/caps from evidence, not star count or theoretical complexity alone.
