...
| SW08       | Body-height sample vision, bounded frozen-pose cache, range-first 3D hearing/transmission, event origins, geometry/sense invalidation, private future-route removal | Muffled/unknown-source contacts and graded detail through EPR, room acoustics, concentrated exposure/audience scaling.                                                                                         |
...
- [ ] **SW06.2a** Before increasing the current geometry/map limits, qualify cold graph preparation against the host's responsiveness budget; consider prewarming or bounded worker preparation under SW06 before increasing bounds. A near-limit manual scene still takes about 160 ms to build/query cold, versus about 3.8 seconds before indexing. A native cache is not a promise of hitch-free large maps.
- [ ] **SW08.4a** Integrate EPR02/EPR10 and PF population work for concentrated first exposure: native runtime profiling still spends about 247 ms creating 524 awareness/event changes with 250 added objects. Do not silently drop evidence or throttle physical safety to hide this. Measure candidate enumeration, emitted events, duplicate-source admission and audience persistence separately.
