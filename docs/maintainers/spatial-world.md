...
Partial delivery, not completion of the broad task: SW03.1–SW04.4 have finite surface/box/ray/sweep support but not arbitrary triangle/convex bodies or large-world spatial indexing. SW05.1/SW05.3–SW05.5 have cached native graphs, exact semantic points and bounded status, not tiled Recast or hidden/remembered map providers. SW07 has next-segment safety and dead/incapacitated bird descent, but no general support-removing gameplay, multi-actor avoidance or moving structures. SW08.2 uses intelligible-only 3D transmission; muffled unidentified speech awaits EPR rather than leaking exact content. SW09.1 has pure camera controls and a browser scenario awaiting execution; SW09.3–SW09.4 viewport leases/multi-view policy are not claimed. SW02.3 retains documented `map.height` as horizontal depth rather than renaming every established map field; arbitrary static rotations remain future work. SW10 supplies a narrow renderer interface and sprite/simple-mesh representations; generic GLB import, directional rigs, complete replacement browser evidence and separate render-view minimization remain. SW12 delivers one prevalidated authored corridor routine, not arbitrary graph generation, general mixed-mode planning or invented biological capability. SW13 generic spatial authoring and late model assets remain unimplemented. Existing recipes/agency remain usable independently.

Deferred concrete work belongs to the original task IDs: qualify arbitrary flight landing/fall volumes and changed supports in SW07/SW12; improve route stance coverage/error presentation in SW05; add dynamic body avoidance only for demonstrated crowd problems in SW07.2; implement graded auditory contacts without broadening exact-word audiences in SW08.2/EPR; resolve the existing D51 viewport details before SW09.3; measure large-world budgets and introduce worker/BVH/navmesh infrastructure only when SW14 shows need. The current finite provider fails closed on invalid/over-limit geometry and does not expose arbitrary geometry mutation through the UI.
...
- [ ] **SW03.4** Build the elevation fixture described below from canonical geometry, not decorative sprites. Verify ramps, bridge top, underside clearance, walls, and floors have consistent visual/navigation/query sources.
...
- [ ] **SW04.3** Add action-specific contact/stance/line-of-effect predicates. Convert ranged range/occlusion to 3D without pretending the existing probabilistic shot family becomes a ballistic simulator.
...
- [ ] **SW09.1** Add plain camera state and pure orbit/pan/pitch/zoom/recenter controls, orthographic and perspective modes, optional heading snaps/locks, sprite-friendly pitch limits, and 3D focus. Keep camera heading independent of actor heading.
...
- [ ] **SW11.1** Replace y=0 ray-plane selection with authorized surface intersections and explicit destination support IDs. Implement clear upper/lower-floor disambiguation and marker/preview agreement with the server-selected stance.
- [ ] **SW11.2** Add level grouping/focus and roof/upper-floor cutaway presentation without deleting physical geometry. Prevent hidden actor/asset/label payloads from being delivered as a shortcut to client culling.
...
- [ ] **SW16.2** Run focused tests, formatting, typecheck/full repository checks, and browser checks for the delivered slice. Record actual outputs, unrelated failures, and omissions in existing verification/status owners without marking all SW complete.
