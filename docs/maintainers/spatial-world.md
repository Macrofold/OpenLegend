...
| SW04       | Full-extent static AABB trees, exact rays/conservative upright-box sweeps, weak frozen-stance memoization, support queries and current action predicates                                                | Generic capsule/convex interfaces, dynamic body broad phase beyond landing, broader tangent/overflow coverage.                                                                                                                 |
| SW05       | Cached layered A\*, strict surface endpoints, movement-patch sharing, lazy checked edges, connector-aware A\*, exact endpoint seams, exhausted-search proofs, reusable scratch and native stance search | Tiled/arbitrary mesh navigation, hidden/remembered maps, broader stance coverage and accurate application-level budget/unavailable statuses. The current wrapper can collapse distinct failures into no route.                 |
...
| SW08       | Body-height sample vision, bounded frozen-pose cache, range-first 3D hearing/transmission, event origins, geometry/sense invalidation, private future-route removal                                     | General non-speech sound contacts, room acoustics and concentrated exposure scaling remain. Speech now has graded/unknown-source evidence and encounter acquisition is observer-private; broader EPR delivery is not complete. |
...
- [x] **SW05.1a** Remove all-height seam comparisons and add conservative disconnected-support rejection; bound interaction support candidates before expensive reach/stance/path work. Keep actual geometry and route budgets authoritative.
...
- [ ] **SW06.2a** Before increasing geometry/map limits or admitting dense authored scenes, qualify worst-case native route work against the host responsiveness budget. Lazy exact edges and a tighter admissible heuristic remove much eager construction, but a first exhaustive failure among slightly different overlapping supports still took about 392 ms in a manual diagnostic. Identical-patch sharing reduces that specific repeated geometry; it does not certify general connected-world latency. Repeated proved failures reuse bounded closed-region facts; incomplete searches must not be cached as impossible. Prewarming/bounded preparation or a qualified representation remains required if the actual workload violates its latency budget. Historical eager-build timings belong in Verification, not current capacity claims.
- [ ] **SW08.4a** Integrate EPR02/EPR03/EPR10 and PF population work for concentrated first exposure. Observer-private encounter acquisition, indexed experience admission and exact geometry reuse are delivered; the former external rebroadcast is removed. Every genuine occurrence/recipient still requires evidence storage. Preserve living-encounter and narration semantics while completing broader exposure-delta work. Qualify large memory-capable populations, persistence and all-to-all hotspots; do not drop evidence or weaken physical safety to hide overload.
...
- [x] **SW05.1c** Evaluate exact stance/edges lazily over bounded support metadata, use the current cardinal-grid/endpoint-aware admissible heuristic and deterministic progress tie-break, and retain at most four closed-region negative proofs from fully exhausted searches. Preserve directional seam checks, support IDs, saved routes and budget failure distinctions. SR11 owns deferred automated coverage.

- [x] **SW04.1c** Memoize exact stances for frozen points using weak keys and current radius/height/slope inputs under prepared map/revision geometry. Mutable points retain complete validation; automated qualification remains SR14.
- [x] **SW05.1d** Share navigation for identical public movement patches while preserving physical/acoustic records and explicit endpoint support seams. Retain existing lazy edge evaluation, admissible heuristic and four exhausted closed-region proofs rather than adding another failure cache. Automated qualification remains SR13–SR14.
