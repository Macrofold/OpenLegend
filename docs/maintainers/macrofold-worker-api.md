...
- [x] Adapt the pre-existing Sandbox-based fixture in `apps/server/src/cognition.test.ts` to the Worker contract (review fix; otherwise the existing CI test gate fails). It was not run locally; CI remains the gate. After rebasing onto main, full `pnpm typecheck` has zero diagnostics.
