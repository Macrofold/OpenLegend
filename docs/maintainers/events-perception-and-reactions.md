...
[Hearing and action branches](remaining-foundational-work.md#branch-coverage-snapshot) implement meaningful parts of event-time evidence and admission; do not duplicate them. EPR01/EPR03–EPR07 still need common typed occurrence → scoped evidence → change/reaction integration for non-speech senses, internal persistent appraisals and witnessed departure/return. Keep raw physical occurrence, actor knowledge, optional wakeup and paid execution distinct. Integrate EPR08 cursors/episodes through SL00 and current data owners; EPR09 must measure dense actual recipients and cold retained history together with cadence, not just isolated speech throughput. Preserve all native outcomes and authorized evidence when coalescing optional work.
