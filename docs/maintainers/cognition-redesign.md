...
- [ ] Integrate the [D2 canonical source/index migration](production-data.md#delivery-slices-and-exit-evidence) and [database retrieval contract](../../archive/07-technical-architecture/data-queries-and-mcp.md#relevance-retrieval-without-whole-history-preparation). Verify actor/timeline/source revisions, required evidence during index lag, correction/forgetting and bounded result preparation; qualify the 20 ms target under D62. Existing database-side vector comparison does not establish whole-history database retrieval or complete indexing.
