...
- [ ] Add automated coverage for conversation-aware stale-decision rejection, mandatory-budget rejection before embeddings, zero-remaining-budget retrieval, action-space reservation before Jev, and large-backlog candidate construction. Reconcile the cognition documentation after runtime acceptance. Tests and documentation were deferred at the user's request.
- [ ] Add automated coverage for importance/urgency interruption policy and the single response refresh: ordinary awareness must not delay replies; only events at or above both thresholds refresh once; the retry includes interrupting evidence and uses distinct provider identities; a second interruption does not loop. Cover legacy awareness without urgency and reconcile the cognition documentation. Tests and documentation were deferred at the user's request.
- [ ] Add automated coverage for shared context-list attention: each section with at most 24 optional entries bypasses embeddings, larger sections use ranked semantic top-24, all section finalists share one batched Jev judgment, and only judged entries fitting the global byte balance render. Cover memories, surroundings, inventory, knowledge, vector lag/fallback and per-section diagnostics. Tests and documentation were deferred at the user's request.
