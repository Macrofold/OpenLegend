...
- [x] Reconcile conversation/retrieval documentation for the interim participant-pair/time-gap boundary, the newest-32 plus semantic-32 prompt policy, retained-history independence, required-versus-semantic byte allocation and eventual replacement by durable conversation IDs.
- [ ] Replace the interim pair/time-gap boundary with durable `conversation_id` membership when NC05 lands. The current runtime can guarantee actor-aware speech between the addressed pair, but cannot associate untargeted actions or ambient events with a conversation until event conversation binding exists.
...
- [ ] Add automated coverage for importance/urgency interruption policy and the single response refresh: ordinary awareness must not delay replies; only events at or above both thresholds abort the active provider stage and refresh once; the retry includes interrupting evidence and uses distinct provider/application-stage identities; a second interruption does not loop. Cover legacy awareness without urgency and ensure user, pause and shutdown cancellation cannot become a retry. Automated tests were deferred at the user's request.
- [ ] Add automated coverage for shared context-list attention: each ordinary section with at most 24 optional entries bypasses embeddings, larger sections use ranked semantic top-24, older conversation speech uses top 32, all optional section finalists share one batched Jev judgment, and only judged entries fitting the global byte balance render. Cover memories, conversation, surroundings, inventory, knowledge, vector lag/fallback and per-section diagnostics. Tests were deferred at the user's request.
...
- [ ] Add automated regression coverage for conversational context races: ordinary awareness changes do not invalidate a response; the first newly perceived event meeting both importance and urgency thresholds refreshes context once with distinct admitted provider IDs; another qualifying event cannot loop. Pause races wait and commit the generated response after resume. Provider, schema, storage, actor-availability and spending failures remain terminal and visible only through the message-local **Failed** state.
...
- [ ] Add regression coverage that `replyInterruption`-style subtitles cannot reappear: cancelled, stale and failed terminal reply jobs project only as message-local **Failed** with hover/focus detail; successful messages show no technical caption, including partially rejected response components.
