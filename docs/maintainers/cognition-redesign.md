...
- [x] Separate raw-recall and request limits, authored-text quotas, presentation-thought bounds and native commitment creation limits. Retained summary counts and consolidation pressure do not gate native simulation.
...
- [x] Integrate the [D2 canonical source/index migration](production-data.md#remaining-d1d2-implementation-and-evidence), SQL eligibility, required-evidence paths, index coverage, durable claims and bounded hydration. Native component measurements and source/forgetting/restore drills are in [Verification](../verification.md#data-foundation-runtime); broad workload and behavioral acceptance remain open. The arbitrary 20 ms target is withdrawn.
...
- [x] Bound maintenance requests and dispatch at accelerated speeds; preserve sources on failure without automatic paid reruns. Delayed consolidation never pauses native simulation or reports a save error. Older retained sources remain queryable without dumping them into conversation.
...
- [ ] Build semantic classification/grouping and dream behavior on D2's [source-fidelity contract](../../archive/07-technical-architecture/production-data-model.md#semantic-history-and-grouping-support): separate routine motion from meaningful encounters/harm, keep repeated consequential incidents distinguishable, preserve attribution and protected evidence through grouping/reclassification, and record any source-loss policy in the [retention ledger](../../archive/07-technical-architecture/data-delivery-and-scale.md#retention-decision-ledger). The foundation supplies storage capability, not proof of semantic quality or a replacement consolidation algorithm.
...
Inactive-history integration is delivered: maintenance reads an actor-scoped durable snapshot and validates hydrated sources when publishing; optional memory contexts select from SQL. No new semantic policy or live-model acceptance is implied. [PF08](performance.md#pf08--long-lived-worlds-hot-state-and-checkpoints) owns remaining full-backlog materialization cost; [native evidence](../verification.md#data-runtime-hardening) includes cold-source consolidation and restart.

- [ ] Qualify existing fixtures at runtime and add transaction isolation, concurrent world commands, budget admission, rollback/disconnection and shutdown coverage when authorized. Async API and current-contract fixture typing is repaired and full typecheck passes; [native hardening drills](../verification.md#data-runtime-hardening) cover selected failure/concurrency cases. No automated suites were run.
- [ ] Add consolidation coverage for explicit source partitions, updating existing memory identities, unrelated routine incidents, unchanged individual important incidents, missing/duplicate/stale sources, request-size preflight, hourly same-day batches, daily review of every consolidated and remaining raw memory from the previous completed day, and cancellation/provider failure preserving originals. Reject reordered groups and groups spanning an intervening memory. Verify useful recall from overdue raw sources through the SQL retained-source selection path and independent required-evidence selection.
...
- [x] Reject selected evidence when forgetting/correction, exact source version or restore generation changes during attention. Required/fallback selections share this boundary; grouped candidates validate every contributing source, including members whose ID differs from the group's ID. Native editor/retrieval reruns are in [foundation review evidence](../verification.md#data-foundation-review). Automated delayed-provider coverage, including editing a secondary group member while attention waits, remains part of the existing deferred validation gate; this is implementation, not live behavioral acceptance.
...
## Context capacity and snapshot intake

- [x] Preserve ordered hard-query baselines on every trigger and optional relevance failure; replace question-count caps with serialized size admission.
- [x] Retrieve across the complete permitted action library for top-300 relevance and expose coverage/size omissions.
