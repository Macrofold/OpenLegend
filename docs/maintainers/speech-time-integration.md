...
Reinspected `feat/hearing-speech-captions` at `f1ee3ffb799239888eb0e8e025cf4f5193e50198`, then its two-commit update through `9802e5c5dcdeb01de8634d471160464d1835799a` (draft PR #3), alongside the spatial elapsed implementation integrated with main `03105fed`, including the `fdcbd31` rules. Recheck tips before integration; this is not permission to overwrite newer work. The sound branch was read, not modified or merged by the cadence task.
...
The newer sound branch additionally retains listener geometry across nonspatial actor changes, indexes resource definitions for urgent native foraging, and measures actual disk-backed 8x speech delivery through `scripts/stress-hearing.ts`. Its default-world visual discovery policy samples every four game seconds while retaining one-second physical steps. Those two choices are not the elapsed contract: do not reinstate the physical loop or blindly stack the visual timer on top of spatial displacement bounds. Decide the unified observation policy explicitly in its world owner. Both branches independently allocated **RP03** to different cadence/discovery policies; preserve both decisions under distinct stable entries during integration rather than overwriting one.
...
- Native participant/supply indexes: retain sound's resource-definition candidate index and exact current quantity/visibility checks when adapting survival to interval start boundaries. Do not restore repeated whole-scenery foraging scans or add a competing supply cache.
...
An integration note was posted on PR #3 (comment 5827327924), pointing out the obsolete 480-native-steps assumption, the shared helpers and the recommended merge order. The branch was inspected, not overwritten or merged by this task. The updated PR now states in-place preservation and distinguishes its short selected 8x workload from the still-failing dense population. Those are hearing-branch measurements, not qualification of the joined Recast/elapsed/hearing implementation. Recheck its tip before joining the branches and retain HE05's unqualified graphical/provider/scale gates.
