...
## Implemented authoring sessions

The application now stores world/timeline/account-bound authoring sessions, immutable draft revisions, prepared changes and exact human approvals in existing operational metadata. Recipes, native attribute declarations and existing native commands are implemented kind adapters. Creator recipe installation uses the normal validator and owner writer without an inhabitant, automatic teaching or object creation. Ordinary actor invention remains actor-scoped. These finite adapters do not implement general module composition, shared-law migration or new physical families.

Draft changes use expected revisions and exact operation fingerprints. Preparing a change runs an unpaid native preview and retains source-dependency, manifest, policy and timeline pins. Apply requires the matching human-approved plan; the agent cannot call the human approval mutation. Guards run again inside the native writer. An `applying` record after interruption first reconciles the original native receipt, rather than applying a replacement snapshot or buying another call. Committed action admission does not establish terminal action success.

Sessions are outside gameplay rewind and retain the original allowance. Reopening does not create another budget. The current single-host owner path uses exact-key metadata, bounded draft/plan/revision counts and batched list reads; it is not a collaborative or distributed authoring store. Full draft bytes remain separately inspectable. Budget summaries use the existing indexed attempts/root-membership ledger, not another wallet. The native-harness and image dispatch integrations are qualified separately from zero-call authoring operations.
