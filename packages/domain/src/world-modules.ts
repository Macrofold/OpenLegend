...
export function validateWorldModules(world: WorldState): void {
  if (world.schemaVersion !== 10 || !world.moduleManifest)
    throw new Error(
      'Incompatible development world schema. This build requires movement format 10; use a separate OPEN_LEGEND_DATA_DIR for a fresh world. Existing saves are preserved, not migrated.',
    );
