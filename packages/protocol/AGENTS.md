...
For a wire change, trace [server admission](../../apps/server/src/world-service.ts), projection, serialization/patch application and all client consumers together. Never solve a missing client field by exposing raw world state or ungranted private data. Keep absent, unknown, unsupported and empty values semantically distinct; do not silently coerce incompatible payloads.
