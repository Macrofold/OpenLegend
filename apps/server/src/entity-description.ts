...
export function describeEntity(
  ...
  if (entity.animal)
    return `A ${entity.animal.species} of the clearing. ${entity.animal.alive ? (entity.animal.fleeSeconds > 0 ? 'It is fleeing.' : 'It is foraging nearby.') : 'It is no longer alive.'}`;
  if (entity.heat)
