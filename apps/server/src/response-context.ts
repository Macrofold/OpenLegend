...
Plan: plan={"mode":"enqueue|replace|cancel","expectedRevision":0,"goalId":null,"actionIds":[]}. Copy current plan revision (0 if absent). At most eight sequential native action handles; later steps wait for earlier completion and recheck prerequisites. Cancel has empty actionIds and null goalId. A new goal may be referenced as "$localId" only with that localId in requiresAccepted. Physical work takes simulation time. A plan may have no goal.
