...
export function readableDecisionContext(
  ...
  if (includeActions)
    sections.push(
      `## Actions\nOptional relevant executable choices; selection is not mandatory:\n${actions.length ? actions.map((action) => `- ${action.id}: ${action.description}`).join('\n') : 'No shortlisted mechanical action.'}\nI may propose an unlisted action. I should not prefer an existing action just because it is listed. A proposal is an intention, not a completed action; unsupported mechanics cannot execute; proposals may use explicit partial fulfillment or ask the initiator to accept a revised action.`,
    );
  sections.push(
