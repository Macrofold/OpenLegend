...
export class KnowledgeStore {
  ...
  async project(before: WorldState | undefined, after: WorldState, restore = false) {
    if (restore) await this.db.prepare('DELETE FROM knowledge_documents WHERE world_id=?').run(after.id);
    ...
    for (const actorId of actors) {
      ...
      for (const id of Object.keys(prior ?? {}))
        if (!next?.[id]) await this.db.prepare('DELETE FROM knowledge_documents WHERE world_id=? AND actor_id=? AND document_id=?').run(after.id, actorId, id);
      for (const [id, doc] of Object.entries(next ?? {})) {
        if (prior?.[id] === doc) continue;
        await this.db.prepare(`INSERT INTO knowledge_documents VALUES (?,?,?,?,?,?)
  ...
  async verify(world: WorldState) {
    const rows = await this.db.prepare('SELECT actor_id,document_id,payload FROM knowledge_documents WHERE world_id=?').all(world.id);
    const expected = new Map(Object.entries(world.actorKnowledge ?? {}).flatMap(([actor, docs]) =>
      Object.entries(docs).map(([id, doc]) => [`${actor}\0${id}`, JSON.stringify(doc)] as const)));
    if (rows.length !== expected.size || rows.some(row => expected.get(`${row['actor_id']}\0${row['document_id']}`) !== row['payload']))
      throw new Error('Accepted knowledge rows disagree with their world commit; reconcile before starting.');
  }
}
