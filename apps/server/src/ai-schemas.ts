...
export const declarationSchema: JsonSchema = object({
  schemaVersion: { type: 'integer', const: DECLARATION_CONTRACT.schemaVersion },
  name: text(80),
  ...
  inputs: {
    ...
    items: object({
      ...
      quantity: {
        type: 'integer',
        minimum: DECLARATION_CONTRACT.inputQuantity.minimum,
        maximum: DECLARATION_CONTRACT.inputQuantity.maximum,
      },
      role: { type: 'string', enum: Object.keys(DECLARATION_CONTRACT.roleProperties) },
    }),
  },
  workSeconds: {
    type: 'integer',
    minimum: DECLARATION_CONTRACT.workSeconds.minimum,
    maximum: DECLARATION_CONTRACT.workSeconds.maximum,
  },
  output: object({
    kind: { type: 'string', enum: [...DECLARATION_CONTRACT.outputKinds] },
    gatheringTool: nullable(
      object({
        resourceId: text(120),
        quantity: {
          type: 'integer',
          minimum: DECLARATION_CONTRACT.gatheringTool.quantity[0],
          maximum: DECLARATION_CONTRACT.gatheringTool.quantity[1],
        },
      }),
    ),
    name: text(80),
