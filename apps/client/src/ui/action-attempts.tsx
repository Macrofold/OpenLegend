...
export function ActionAttempts({ view, connected }: { view: GameView; connected: boolean }) {
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem(`open-legend:action-draft:${view.worldId}:${view.saveTimeline}`, text);
    } catch {
    ...
    const refresh = async () => {
      try {
        const result = await post<{
          ok: boolean;
          attempts: PlayerActionAttempt[];
          job?: { status: string; message: string; result?: { fulfillment?: unknown } };
        }>('/api/action-attempts', { requestId });
        if (stopped || !result.ok) return;
        setAttempts(result.attempts);
        if (result.job) setMessage(result.job.message);
      } catch {
        /* Keep current display during transient disconnection. */
      }
    };
    void refresh();
  ...
  const send = async () => {
    ...
    try {
      ...
      if (result.ok) {
        setRequestId(submission.current.id);
        submission.current = null;
      }
    } catch (error) {
      if (alive.current)
  ...
  return (
    <Section title="Take an action">
      ...
      <Button
        disabled={busy || !connected || view.clock.paused || !text.trim()}
        onPress={() => void send()}
      >
        Attempt action
      </Button>
      ...
      {attempts.map((attempt) => (
        <div className="ol-proposal" key={attempt.id}>
          ...
          {attempt.fulfillment ? (
            ...
            <p className="ol-caption">
              No executable binding yet. Revise the request or withdraw it.
            </p>
          )}
          <Button
            variant="quiet"
