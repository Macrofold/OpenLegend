...
Known recipes and invention jobs have structured cards. Prose world-agent replies ending in a question receive an answer affordance; this is presentation, not a typed multi-question backend protocol. The owner World Agent is a unified funded native-harness conversation with durable history, exact review and Apply controls. Missing connector configuration does not silently fall back to tool-less chat. Earlier local transcripts are read-only history. The labeled native recipe shortcut optionally offers Review in workshop before installing, a free capability catalogue, saved native validation details, and an explicit Apply saved proposal action. This finite recipe path does not edit shared laws or existing objects. General creator-module authoring, conjure confirmation and generated artwork remain in INV; UI styling does not grant those capabilities. Ending a conversation closes its backend lane; restoring its transcript creates a new identity, never pretends to reopen a tombstoned session. Hiding a panel retains drafts and pending work.
